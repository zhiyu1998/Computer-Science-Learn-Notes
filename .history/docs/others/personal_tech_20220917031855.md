...
在分页之前我们首先要做的是定义字符串的优先级：
```java
public static final HashMap<String, Integer> managePriority = new HashMap<>();
    static {
        managePriority.put(PlanStatusEnums.UN_AUDIT.name(), 1);
        managePriority.put(PlanStatusEnums.UN_PLAN.name(), 2);
        managePriority.put(PlanStatusEnums.PLANNED.name(), 3);
        managePriority.put(PlanStatusEnums.NO_STOCK.name(), 4);
    }
```
### 分页后排序（每页排序）
