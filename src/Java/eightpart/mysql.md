...
**Binlog（二进制日志）**：它记录了对数据库执行的所有数据修改操作，如INSERT，UPDATE，DELETE等，但不包括SELECT和SHOW等查询指令。主要用于复制和恢复。在复制中，主数据库的binlog日志会被复制到从数据库执行以达到数据的一致性。在恢复中，只要有完整的备份加上之后的所有binlog日志，就可以恢复到任何一个点的数据。Binlog日志是按照时间先后顺序记录，事件一旦写入就不会改变。

**Undolog（回滚日志）**：在InnoDB存储引擎中，undolog用于存储事务执行的逆向操作，它保证了数据库在发生错误或系统崩溃时可以进行正确的回滚操作，恢复到事务开始的状态，以此来保证事务的ACID特性中的一致性和隔离性。undolog以事务为单位进行记录，如果一个事务中执行了多条语句，那么这些语句的逆向操作都会存储在同一个undolog中。

**Redolog（重做日志）**：Redolog也是InnoDB存储引擎特有的日志系统，它主要用于恢复操作和保护数据的完整性。当事务提交时，只需要将事务的修改操作记录到redolog中即可，不需要立即修改实际的数据文件，这样可以极大提高事务的执行效率。此外，如果系统发生崩溃，可以通过重做redolog中记录的操作来恢复数据，保证数据的完整性。和binlog不同的是，redolog是循环写入的，空间是有限的。
