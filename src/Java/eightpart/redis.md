...
1. 快照持久化是 Redis 默认采用的持久化方式，在 `redis.conf` 配置文件中默认有此下配置：
...
2. 其二是通过在客户端执行`bgsave`命令显式触发一次RDB快照的执行：
![](./personal_images/QQ截图20230623205725.webp)
...
> 概要：redis`缓存穿透`、缓存击穿和缓存雪崩都是Redis缓存中的问题。其中，缓存穿透是指`查询一个不存在的数据`，由于缓存中没有，所以每次请求都会到数据库中查询，这样会对数据库造成很大的压力；`缓存击穿`是指一个key非常热点，在不停的`扛着大并发`，大量的请求同时访问这个key，当这个key在失效的瞬间，持续的大并发就穿破缓存，直接请求数据库；而`缓存雪崩`是指`在某一个时间段，缓存集中过期失效`。这样，在失效的一瞬间，大量的请求直接打到数据库上，导致数据库短时间内承受巨大压力。
> 解决方案：
> - 对于缓存穿透问题，可以采用`布隆过滤器`或者将查询结果为空也写入缓存
> - 对于缓存击穿问题，可以采用`互斥锁`或者使用`分布式锁`
> - 对于缓存雪崩问题，可以采用`加过期时间随机值`或者使用`消息队列`来削峰填谷
...
![img](./personal_images/cluster_slots.webp)
...
![image-20220715143320253](./personal_images/image-20220715143320253.webp)

Redis 维护集群元数据采用另一个方式， `gossip` 协议，所有节点都持有一份元数据，不同的节点如果出现了元数据的变更，就不断将元数据发送给其它的节点，让其它节点也进行元数据的变更。

![image-20220715143327361](./personal_images/image-20220715143327361.webp)
...
![image-20220715143507698](./personal_images/image-20220715143507698.webp)
...
![image-20220715143520445](./personal_images/image-20220715143520445.webp)
...
![image-20220715143532988](./personal_images/image-20220715143532988.webp)
