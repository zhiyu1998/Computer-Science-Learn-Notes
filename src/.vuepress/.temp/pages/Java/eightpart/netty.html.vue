<template><div><h2 id="🕸️netty" tabindex="-1"><a class="header-anchor" href="#🕸️netty" aria-hidden="true">#</a> 🕸️Netty</h2>
<h3 id="reactor模型" tabindex="-1"><a class="header-anchor" href="#reactor模型" aria-hidden="true">#</a> Reactor模型</h3>
...
<p>当一个连接对应一个线程时，线程一般采用「read -&gt; 业务处理 -&gt; send」的处理流程，如果当前连接没有数据可读，那么线程会阻塞在 <code v-pre>read</code> 操作上（ socket 默认情况是阻塞 I/O），不过这种阻塞方式并不影响其他线程。</p>
<p>但是引入了线程池，那么一个线程要处理多个连接的业务，线程在处理某个连接的 <code v-pre>read</code> 操作时，如果遇到没有数据可读，就会发生阻塞，那么线程就没办法继续处理其他连接的业务。</p>
<p>要解决这一个问题，最简单的方式就是将 socket 改成非阻塞，然后线程不断地轮询调用 <code v-pre>read</code> 操作来判断是否有数据，这种方式虽然该能够解决阻塞的问题，但是解决的方式比较粗暴，因为轮询是要消耗 CPU 的，而且随着一个 线程处理的连接越多，轮询的效率就会越低。</p>
<p>上面的问题在于，线程并不知道当前连接是否有数据可读，从而需要每次通过 <code v-pre>read</code> 去试探。</p>
<p>那有没有办法在只有当连接上有数据的时候，线程才去发起读请求呢？答案是有的，实现这一技术的就是 I/O 多路复用。</p>
<p>I/O 多路复用技术会用一个系统调用函数来监听我们所有关心的连接，也就说可以在一个监控线程里面监控很多的连接。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220725152308276.png" alt="image-20220725152308276" tabindex="0" loading="lazy"><figcaption>image-20220725152308276</figcaption></figure>
...
<p>事实上，Reactor 模式也叫 <code v-pre>Dispatcher</code> 模式，我觉得这个名字更贴合该模式的含义，即 <strong>I/O 多路复用监听事件，收到事件后，根据事件类型分配（Dispatch）给某个进程 / 线程</strong>。</p>
...
<h4 id="reactor" tabindex="-1"><a class="header-anchor" href="#reactor" aria-hidden="true">#</a> Reactor</h4>
<h5 id="单-reactor-单进程-线程" tabindex="-1"><a class="header-anchor" href="#单-reactor-单进程-线程" aria-hidden="true">#</a> 单 Reactor 单进程 / 线程</h5>
<p>一般来说，C 语言实现的是「<strong>单 Reactor *单进程*</strong>」的方案，因为 C 语编写完的程序，运行后就是一个独立的进程，不需要在进程中再创建线程。</p>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220725152351105.png" alt="image-20220725152351105" tabindex="0" loading="lazy"><figcaption>image-20220725152351105</figcaption></figure>
...
<h5 id="单-reactor-多线程-多进程" tabindex="-1"><a class="header-anchor" href="#单-reactor-多线程-多进程" aria-hidden="true">#</a> 单 Reactor 多线程 / 多进程</h5>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220725152423113.png" alt="image-20220725152423113" tabindex="0" loading="lazy"><figcaption>image-20220725152423113</figcaption></figure>
<p>详细说一下这个方案：</p>
<ul>
<li>Reactor 对象通过 select （IO 多路复用接口） 监听事件，收到事件后通过 dispatch 进行分发，具体分发给 Acceptor 对象还是 Handler 对象，还要看收到的事件类型；</li>
<li>如果是连接建立的事件，则交由 Acceptor 对象进行处理，Acceptor 对象会通过 accept 方法 获取连接，并创建一个 Handler 对象来处理后续的响应事件；</li>
...
<h5 id="多-reactor-多进程-线程" tabindex="-1"><a class="header-anchor" href="#多-reactor-多进程-线程" aria-hidden="true">#</a> 多 Reactor 多进程 / 线程</h5>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220725152443546.png" alt="image-20220725152443546" tabindex="0" loading="lazy"><figcaption>image-20220725152443546</figcaption></figure>
<p>方案详细说明如下：</p>
<ul>
...
<h4 id="proactor" tabindex="-1"><a class="header-anchor" href="#proactor" aria-hidden="true">#</a> Proactor</h4>
<p>前面提到的 Reactor 是非阻塞同步网络模式，而 <strong>Proactor 是异步网络模式</strong>。</p>
<p>这里先给大家复习下阻塞、非阻塞、同步、异步 I/O 的概念。</p>
<p>先来看看<strong>阻塞 I/O</strong>，当用户程序执行 <code v-pre>read</code> ，线程会被阻塞，一直等到内核数据准备好，并把数据从内核缓冲区拷贝到应用程序的缓冲区中，当拷贝过程完成，<code v-pre>read</code> 才会返回。</p>
<p>注意，<strong>阻塞等待的是「内核数据准备好」和「数据从内核态拷贝到用户态」这两个过程</strong>。过程如下图：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220725152513494.png" alt="image-20220725152513494" tabindex="0" loading="lazy"><figcaption>image-20220725152513494</figcaption></figure>
<p>知道了阻塞 I/O ，来看看<strong>非阻塞 I/O</strong>，非阻塞的 read 请求在数据未准备好的情况下立即返回，可以继续往下执行，此时应用程序不断轮询内核，直到数据准备好，内核将数据拷贝到应用程序缓冲区，<code v-pre>read</code> 调用才可以获取到结果。过程如下图：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220725152523692.png" alt="image-20220725152523692" tabindex="0" loading="lazy"><figcaption>image-20220725152523692</figcaption></figure>
<p>注意，<strong>这里最后一次 read 调用，获取数据的过程，是一个同步的过程，是需要等待的过程。这里的同步指的是内核态的数据拷贝到用户程序的缓存区这个过程。</strong></p>
<p>举个例子，如果 socket 设置了 <code v-pre>O_NONBLOCK</code> 标志，那么就表示使用的是非阻塞 I/O 的方式访问，而不做任何设置的话，默认是阻塞 I/O。</p>
...
<p>当我们发起 <code v-pre>aio_read</code> （异步 I/O） 之后，就立即返回，内核自动将数据从内核空间拷贝到用户空间，这个拷贝过程同样是异步的，内核自动完成的，和前面的同步操作不一样，<strong>应用程序并不需要主动发起拷贝动作</strong>。过程如下图：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220725152532905.png" alt="image-20220725152532905" tabindex="0" loading="lazy"><figcaption>image-20220725152532905</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220725152542296.png" alt="image-20220725152542296" tabindex="0" loading="lazy"><figcaption>image-20220725152542296</figcaption></figure>
...
<p>可惜的是，在 Linux 下的异步 I/O 是不完善的， <code v-pre>aio</code> 系列函数是由 POSIX  定义的异步操作接口，不是真正的操作系统级别支持的，而是在用户空间模拟出来的异步，并且仅仅支持基于本地文件的 aio 异步操作，网络编程中的  socket 是不支持的，这也使得基于 Linux 的高性能网络程序都是使用 Reactor 方案。</p>
<p>而 Windows 里实现了一套完整的支持 socket 的异步编程接口，这套接口就是 <code v-pre>IOCP</code>，是由操作系统级别实现的异步 I/O，真正意义上异步 I/O，因此在 Windows 里实现高性能网络程序可以使用效率更高的 Proactor 方案。</p>
</div></template>
