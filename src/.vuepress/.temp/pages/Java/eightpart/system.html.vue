<template><div><h2 id="💻操作系统" tabindex="-1"><a class="header-anchor" href="#💻操作系统" aria-hidden="true">#</a> 💻操作系统</h2>
<h3 id="什么是协程" tabindex="-1"><a class="header-anchor" href="#什么是协程" aria-hidden="true">#</a> 什么是协程？</h3>
<p>协程是一种用户态的轻量级线程。</p>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220626201137158.png" alt="image-20220626201137158" tabindex="0" loading="lazy"><figcaption>image-20220626201137158</figcaption></figure>
<p>从上图可以看出：一个进程中可以有多个线程，多个线程共享进程的<strong>堆</strong>和**方法区 (JDK1.8 之后的元空间)*<em>资源，但是每个线程有自己的*<em>程序计数器</em></em>、<strong>虚拟机栈</strong> 和 <strong>本地方法栈</strong>。</p>
...
<h3 id="进程有哪几种状态" tabindex="-1"><a class="header-anchor" href="#进程有哪几种状态" aria-hidden="true">#</a> 进程有哪几种状态?</h3>
<p>👨‍💻<strong>面试官</strong> ： 那你再说说<strong>进程有哪几种状态?</strong></p>
<p>🙋 <strong>我</strong> ：我们一般把进程大致分为 5 种状态，这一点和线程很像！</p>
<ul>
...
<h3 id="进程间的通信方式" tabindex="-1"><a class="header-anchor" href="#进程间的通信方式" aria-hidden="true">#</a> 进程间的通信方式</h3>
...
<h3 id="线程间的同步的方式" tabindex="-1"><a class="header-anchor" href="#线程间的同步的方式" aria-hidden="true">#</a> 线程间的同步的方式</h3>
...
<h3 id="进程的调度算法" tabindex="-1"><a class="header-anchor" href="#进程的调度算法" aria-hidden="true">#</a> 进程的调度算法</h3>
...
<h3 id="死锁的四个条件" tabindex="-1"><a class="header-anchor" href="#死锁的四个条件" aria-hidden="true">#</a> 死锁的四个条件</h3>
...
<h3 id="零拷贝深入浅出" tabindex="-1"><a class="header-anchor" href="#零拷贝深入浅出" aria-hidden="true">#</a> ⭐零拷贝深入浅出</h3>
<h4 id="dma" tabindex="-1"><a class="header-anchor" href="#dma" aria-hidden="true">#</a> DMA</h4>
...
<p>为了方便你理解，我画了一副图：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220702132804386.png" alt="image-20220702132804386" tabindex="0" loading="lazy"><figcaption>image-20220702132804386</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220702132824828.png" alt="image-20220702132824828" tabindex="0" loading="lazy"><figcaption>image-20220702132824828</figcaption></figure>
<p>具体过程：</p>
<ul>
...
<h4 id="mmap-write" tabindex="-1"><a class="header-anchor" href="#mmap-write" aria-hidden="true">#</a> mmap + write</h4>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220702133239391.png" alt="image-20220702133239391" tabindex="0" loading="lazy"><figcaption>image-20220702133239391</figcaption></figure>
<p>具体过程如下：</p>
<ul>
...
<h4 id="sendfile" tabindex="-1"><a class="header-anchor" href="#sendfile" aria-hidden="true">#</a> sendfile</h4>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220702133308560.png" alt="image-20220702133308560" tabindex="0" loading="lazy"><figcaption>image-20220702133308560</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220702133318801.png" alt="image-20220702133318801" tabindex="0" loading="lazy"><figcaption>image-20220702133318801</figcaption></figure>
...
<h4 id="使用零拷贝技术的项目" tabindex="-1"><a class="header-anchor" href="#使用零拷贝技术的项目" aria-hidden="true">#</a> 使用零拷贝技术的项目</h4>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220702133411785.png" alt="image-20220702133411785" tabindex="0" loading="lazy"><figcaption>image-20220702133411785</figcaption></figure>
...
<h3 id="僵尸进程和孤儿进程" tabindex="-1"><a class="header-anchor" href="#僵尸进程和孤儿进程" aria-hidden="true">#</a> 僵尸进程和孤儿进程</h3>
<h4 id="僵尸进程" tabindex="-1"><a class="header-anchor" href="#僵尸进程" aria-hidden="true">#</a> 僵尸进程</h4>
<p>一个子进程结束后，它的父进程并没有等待它（调用wait或者waitpid），那么这个子进程将成为一个僵尸进程。僵尸进程是一个已经死亡的进程，但是并没有真正被销毁。它已经放弃了几乎所有内存空间，没有任何可执行代码，也不能被调度，仅仅在进程表中保留一个位置，记载该进程的进程ID、终止状态以及资源利用信息(CPU时间，内存使用量等等)供父进程收集，除此之外，僵尸进程不再占有任何内存空间。这个僵尸进程可能会一直留在系统中直到系统重启。</p>
<blockquote>
<p>《百度百科》解释：僵尸进程是当子进程比父进程先结束，而父进程又没有回收子进程，释放子进程占用的资源，此时子进程将成为一个僵尸进程。如果父进程先退出 ，子进程被init接管，子进程退出后init会回收其占用的相关资源</p>
</blockquote>
<p>危害：占用进程号，而系统所能使用的进程号是有限的；占用内存。</p>
<p>以下情况不会产生僵尸进程：</p>
<ul>
<li>该进程的父进程先结束了。每个进程结束的时候，系统都会扫描是否存在子进程，如果有则用Init进程接管，成为该进程的父进程，并且会调用wait等待其结束。</li>
<li>父进程调用wait或者waitpid等待子进程结束（需要每隔一段时间查询子进程是否结束）。wait系统调用会使父进程暂停执行，直到它的一个子进程结束为止。waitpid则可以加入 <code v-pre>WNOHANG</code>(wait-no-hang)选项，如果没有发现结束的子进程，就会立即返回，不会将调用waitpid的进程阻塞。同时，waitpid还可以选择是等待任一子进程（同wait），还是等待指定pid的子进程，还是等待同一进程组下的任一子进程，还是等待组ID等于pid的任一子进程；</li>
<li>子进程结束时，系统会产生 <code v-pre>SIGCHLD</code>(signal-child)信号，可以注册一个信号处理函数，在该函数中调用waitpid，等待所有结束的子进程（注意：一般都需要循环调用waitpid，因为在信号处理函数开始执行之前，可能已经有多个子进程结束了，而信号处理函数只执行一次，所以要循环调用将所有结束的子进程回收）；</li>
<li>也可以用 <code v-pre>signal(SIGCLD, SIG_IGN)</code>(signal-ignore)通知内核，表示忽略 <code v-pre>SIGCHLD</code>信号，那么子进程结束后，内核会进行回收。</li>
</ul>
<h4 id="孤儿进程" tabindex="-1"><a class="header-anchor" href="#孤儿进程" aria-hidden="true">#</a> 孤儿进程</h4>
<blockquote>
<p>百度百科：在操作系统领域中，孤儿进程指的<strong>是在其父进程执行完成或被终止后仍继续运行的一类进程</strong>。这些孤儿进程将被init进程 (进程号为1)所收养，并由init进程对它们完成状态收集工作。</p>
</blockquote>
<p>一个父进程已经结束了，但是它的子进程还在运行，那么这些子进程将成为孤儿进程。孤儿进程会被Init（进程ID为1）接管，当这些孤儿进程结束时由Init完成状态收集工作。</p>
<h3 id="💫i-o-多路复用-select-poll-epoll" tabindex="-1"><a class="header-anchor" href="#💫i-o-多路复用-select-poll-epoll" aria-hidden="true">#</a> 💫I/O 多路复用：select/poll/epoll</h3>
<h4 id="最基本的-socket-模型" tabindex="-1"><a class="header-anchor" href="#最基本的-socket-模型" aria-hidden="true">#</a> 最基本的 Socket 模型</h4>
...
<p>服务端首先调用 <code v-pre>socket()</code> 函数，创建网络协议为 IPv4，以及传输协议为 TCP 的 Socket ，接着调用 <code v-pre>bind()</code> 函数，给这个 Socket 绑定一个 <strong>IP 地址和端口</strong>，绑定这两个的目的是什么？</p>
<ul>
...
<p>绑定完 IP 地址和端口后，就可以调用 <code v-pre>listen()</code> 函数进行监听，此时对应 TCP 状态图中的 <code v-pre>listen</code>，如果我们要判定服务器中一个网络程序有没有启动，可以通过 <code v-pre>netstat</code> 命令查看对应的端口号是否有被监听。</p>
<p>服务端进入了监听状态后，通过调用 <code v-pre>accept()</code> 函数，来从内核获取客户端的连接，如果没有客户端连接，则会阻塞等待客户端连接的到来。</p>
<p>那客户端是怎么发起连接的呢？客户端在创建好 Socket 后，调用 <code v-pre>connect()</code> 函数发起连接，该函数的参数要指明服务端的 IP 地址和端口号，然后万众期待的 TCP 三次握手就开始了。</p>
<p>在  TCP 连接的过程中，服务器的内核实际上为每个 Socket 维护了两个队列：</p>
<ul>
<li>一个是还没完全建立连接的队列，称为 <strong>TCP 半连接队列</strong>，这个队列都是没有完成三次握手的连接，此时服务端处于 <code v-pre>syn_rcvd</code> 的状态；</li>
<li>一个是一件建立连接的队列，称为 <strong>TCP 全连接队列</strong>，这个队列都是完成了三次握手的连接，此时服务端处于 <code v-pre>established</code> 状态；</li>
</ul>
<p>当 TCP 全连接队列不为空后，服务端的 <code v-pre>accept()</code> 函数，就会从内核中的 TCP 全连接队列里拿出一个已经完成连接的  Socket 返回应用程序，后续数据传输都用这个 Socket。</p>
<p>注意，监听的 Socket 和真正用来传数据的 Socket 是两个：</p>
<ul>
<li>一个叫作<strong>监听 Socket</strong>；</li>
<li>一个叫作<strong>已连接 Socket</strong>；</li>
</ul>
<p>连接建立后，客户端和服务端就开始相互传输数据了，双方都可以通过 <code v-pre>read()</code> 和 <code v-pre>write()</code> 函数来读写数据。</p>
<p>至此， TCP 协议的 Socket 程序的调用过程就结束了，整个过程如下图：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220731143809167.png" alt="image-20220731143809167" tabindex="0" loading="lazy"><figcaption>image-20220731143809167</figcaption></figure>
...
<p>文件描述符的作用是什么？每一个进程都有一个数据结构 <code v-pre>task_struct</code>，该结构体里有一个指向「文件描述符数组」的成员指针。该数组里列出这个进程打开的所有文件的文件描述符。数组的下标是文件描述符，是一个整数，而数组的内容是一个指针，指向内核中所有打开的文件的列表，也就是说内核可以通过文件描述符找到对应打开的文件。</p>
<p>然后每个文件都有一个 inode，Socket 文件的 inode 指向了内核中的 Socket 结构，在这个结构体里有两个队列，分别是<strong>发送队列</strong>和<strong>接收队列</strong>，这个两个队列里面保存的是一个个 <code v-pre>struct sk_buff</code>，用链表的组织形式串起来。</p>
...
<p>于是，为了在层级之间传递数据时，不发生拷贝，只用 sk_buff 一个结构体来描述所有的网络包，那它是如何做到的呢？是通过调整 sk_buff 中 <code v-pre>data</code> 的指针，比如：</p>
<ul>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220717161851452.png" alt="image-20220717161851452" tabindex="0" loading="lazy"><figcaption>image-20220717161851452</figcaption></figure>
<h4 id="如何服务更多的用户" tabindex="-1"><a class="header-anchor" href="#如何服务更多的用户" aria-hidden="true">#</a> 如何服务更多的用户？</h4>
...
<h4 id="多进程模型" tabindex="-1"><a class="header-anchor" href="#多进程模型" aria-hidden="true">#</a> 多进程模型</h4>
<p>基于最原始的阻塞网络 I/O， 如果服务器要支持多个客户端，其中比较传统的方式，就是使用<strong>多进程模型</strong>，也就是为每个客户端分配一个进程来处理请求。</p>
<p>服务器的主进程负责监听客户的连接，一旦与客户端连接完成，accept() 函数就会返回一个「已连接 Socket」，这时就通过 <code v-pre>fork()</code> 函数创建一个子进程，实际上就把父进程所有相关的东西都<strong>复制</strong>一份，包括文件描述符、内存地址空间、程序计数器、执行的代码等。</p>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220717163910297.png" alt="image-20220717163910297" tabindex="0" loading="lazy"><figcaption>image-20220717163910297</figcaption></figure>
...
<p>因此，父进程要“善后”好自己的孩子，怎么善后呢？那么有两种方式可以在子进程退出后回收资源，分别是调用 <code v-pre>wait()</code> 和 <code v-pre>waitpid()</code> 函数。</p>
...
<h4 id="多线程模型" tabindex="-1"><a class="header-anchor" href="#多线程模型" aria-hidden="true">#</a> 多线程模型</h4>
<p>既然进程间上下文切换的“包袱”很重，那我们就搞个比较轻量级的模型来应对多用户的请求 —— <strong>多线程模型</strong>。</p>
<p>线程是运行在进程中的一个“逻辑流”，单进程中可以运行多个线程，同进程里的线程可以共享进程的部分资源的，比如文件描述符列表、进程空间、代码、全局数据、堆、共享库等，这些共享些资源在上下文切换时是不需要切换，而只需要切换线程的私有数据、寄存器等不共享的数据，因此同一个进程下的线程上下文切换的开销要比进程小得多。</p>
<p>当服务器与客户端 TCP 完成连接后，通过 <code v-pre>pthread_create()</code> 函数创建线程，然后将「已连接 Socket」的文件描述符传递给线程函数，接着在线程里和客户端进行通信，从而达到并发处理的目的。</p>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220717163933495.png" alt="image-20220717163933495" tabindex="0" loading="lazy"><figcaption>image-20220717163933495</figcaption></figure>
...
<h4 id="i-o-多路复用" tabindex="-1"><a class="header-anchor" href="#i-o-多路复用" aria-hidden="true">#</a> I/O 多路复用</h4>
<p>既然为每个请求分配一个进程/线程的方式不合适，那有没有可能只使用一个进程来维护多个 Socket 呢？答案是有的，那就是 <strong>I/O 多路复用</strong>技术。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220717165055763.png" alt="image-20220717165055763" tabindex="0" loading="lazy"><figcaption>image-20220717165055763</figcaption></figure>
...
<h4 id="select-poll" tabindex="-1"><a class="header-anchor" href="#select-poll" aria-hidden="true">#</a> select/poll</h4>
...
<p>select 使用固定长度的 BitsMap，表示文件描述符集合，而且所支持的文件描述符的个数是有限制的，在 Linux 系统中，由内核中的 FD_SETSIZE 限制， 默认最大值为 <code v-pre>1024</code>，只能监听 0~1023 的文件描述符。</p>
...
<div class="language-c line-numbers-mode" data-ext="c"><pre v-pre class="language-c"><code><span class="token keyword">int</span> s <span class="token operator">=</span> <span class="token function">socket</span><span class="token punctuation">(</span>AF_INET<span class="token punctuation">,</span> SOCK_STREAM<span class="token punctuation">,</span> <span class="token number">0</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
<span class="token keyword">while</span><span class="token punctuation">(</span><span class="token number">1</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>epoll 通过两个方面，很好解决了 select/poll 的问题。</p>
<p><em>第一点</em>，epoll 在内核里使用<strong>红黑树来跟踪进程所有待检测的文件描述字</strong>，把需要监控的 socket 通过 <code v-pre>epoll_ctl()</code> 函数加入内核中的红黑树里，红黑树是个高效的数据结构，增删改一般时间复杂度是 <code v-pre>O(logn)</code>。而 select/poll 内核里没有类似 epoll 红黑树这种保存所有待检测的 socket 的数据结构，所以 select/poll  每次操作时都传入整个 socket 集合给内核，而 epoll 因为在内核维护了红黑树，可以保存所有待检测的 socket  ，所以只需要传入一个待检测的 socket，减少了内核和用户空间大量的数据拷贝和内存分配。</p>
<p><em>第二点</em>， epoll 使用<strong>事件驱动</strong>的机制，内核里<strong>维护了一个链表来记录就绪事件</strong>，当某个 socket 有事件发生时，通过<strong>回调函数</strong>内核会将其加入到这个就绪事件列表中，当用户调用 <code v-pre>epoll_wait()</code> 函数时，只会返回有事件发生的文件描述符的个数，不需要像 select/poll 那样轮询扫描整个 socket 集合，大大提高了检测的效率。</p>
<p>从下图你可以看到 epoll 相关的接口作用：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220717165140152.png" alt="image-20220717165140152" tabindex="0" loading="lazy"><figcaption>image-20220717165140152</figcaption></figure>
...
<p>插个题外话，网上文章不少说，<code v-pre>epoll_wait</code> 返回时，对于就绪的事件，epoll 使用的是共享内存的方式，即用户态和内核态都指向了就绪链表，所以就避免了内存拷贝消耗。</p>
<p>这是错的！看过 epoll 内核源码的都知道，<strong>压根就没有使用共享内存这个玩意</strong>。你可以从下面这份代码看到， epoll_wait 实现的内核代码中调用了 <code v-pre>__put_user</code> 函数，这个函数就是将数据从内核拷贝到用户空间。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220717165148621.png" alt="image-20220717165148621" tabindex="0" loading="lazy"><figcaption>image-20220717165148621</figcaption></figure>
<h5 id="边缘触发和水平触发" tabindex="-1"><a class="header-anchor" href="#边缘触发和水平触发" aria-hidden="true">#</a> 边缘触发和水平触发</h5>
...
<p>如果使用边缘触发模式，I/O 事件发生时只会通知一次，而且我们不知道到底能读写多少数据，所以在收到通知后应尽可能地读写数据，以免错失读写的机会。因此，我们会<strong>循环</strong>从文件描述符读写数据，那么如果文件描述符是阻塞的，没有数据可读写时，进程会阻塞在读写函数那里，程序就没办法继续往下执行。所以，<strong>边缘触发模式一般和非阻塞 I/O 搭配使用</strong>，程序会一直执行 I/O 操作，直到系统调用（如 <code v-pre>read</code> 和 <code v-pre>write</code>）返回错误，错误类型为 <code v-pre>EAGAIN</code> 或 <code v-pre>EWOULDBLOCK</code>。</p>
