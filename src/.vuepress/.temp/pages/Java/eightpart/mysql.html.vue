<template><div><h2 id="🐬-mysql" tabindex="-1"><a class="header-anchor" href="#🐬-mysql" aria-hidden="true">#</a> 🐬 MYSQL</h2>
<h3 id="mysql-基础架构-一条-sql-查询语句是如何执行的" tabindex="-1"><a class="header-anchor" href="#mysql-基础架构-一条-sql-查询语句是如何执行的" aria-hidden="true">#</a> MySql 基础架构 | 一条 SQL 查询语句是如何执行的</h3>
<figure><img src="@source/Java/eightpart/personal_images/image-20220613104147805.png" alt="image-20220613104147805" tabindex="0" loading="lazy"><figcaption>image-20220613104147805</figcaption></figure>
<ul>
...
<h3 id="数据库的索引分类" tabindex="-1"><a class="header-anchor" href="#数据库的索引分类" aria-hidden="true">#</a> 数据库的索引分类</h3>
<ul>
...
<h4 id="按数据结构分类" tabindex="-1"><a class="header-anchor" href="#按数据结构分类" aria-hidden="true">#</a> 按数据结构分类</h4>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220712154456833.png" alt="image-20220712154456833" tabindex="0" loading="lazy"><figcaption>image-20220712154456833</figcaption></figure>
...
<p>举个🌰，有这样一个表</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220712154548420.png" alt="image-20220712154548420" tabindex="0" loading="lazy"><figcaption>image-20220712154548420</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220712154624540.png" alt="image-20220712154624540" tabindex="0" loading="lazy"><figcaption>image-20220712154624540</figcaption></figure>
<h5 id="通过主键查询商品数据的过程" tabindex="-1"><a class="header-anchor" href="#通过主键查询商品数据的过程" aria-hidden="true">#</a> 通过主键查询商品数据的过程</h5>
...
<h5 id="通过二级索引查询商品数据的过程" tabindex="-1"><a class="header-anchor" href="#通过二级索引查询商品数据的过程" aria-hidden="true">#</a> 通过二级索引查询商品数据的过程</h5>
...
<p>我这里将前面的商品表中的 product_no （商品编码）字段设置为二级索引，那么二级索引的 B+Tree 如下图，其中非叶子的 key 值是 product_no（图中橙色部分），叶子节点存储的数据是主键值（图中绿色部分）。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220712154658288.png" alt="image-20220712154658288" tabindex="0" loading="lazy"><figcaption>image-20220712154658288</figcaption></figure>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>会先检二级索引中的 B+Tree 的索引值（商品编码，product_no），找到对应的叶子节点，然后获取主键值，然后再通过主键索引中的 B+Tree 树查询到对应的叶子节点，然后获取整行数据。<strong>这个过程叫「回表」，也就是说要查两个 B+Tree 才能查到数据</strong>。如下图：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220712154711271.png" alt="image-20220712154711271" tabindex="0" loading="lazy"><figcaption>image-20220712154711271</figcaption></figure>
<p>不过，当查询的数据是能在二级索引的 B+Tree 的叶子节点里查询到，这时就不用再查主键索引查，比如下面这条查询语句：</p>
<div class="language-sql line-numbers-mode" data-ext="sql"><pre v-pre class="language-sql"><code><span class="token keyword">select</span> id <span class="token keyword">from</span> product <span class="token keyword">where</span> product_no <span class="token operator">=</span> <span class="token string">'0002'</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><strong>这种在二级索引的 B+Tree 就能查询到结果的过程就叫作「覆盖索引」，也就是只需要查一个 B+Tree 就能找到数据</strong>。</p>
<h4 id="按物理存储分类" tabindex="-1"><a class="header-anchor" href="#按物理存储分类" aria-hidden="true">#</a> 按物理存储分类</h4>
...
<h4 id="按字段特性分类" tabindex="-1"><a class="header-anchor" href="#按字段特性分类" aria-hidden="true">#</a> 按字段特性分类</h4>
<p>从字段特性的角度来看，索引分为主键索引、唯一索引、普通索引、前缀索引。</p>
<ul>
<li><strong>主键索引</strong>就是建立在主键字段上的索引，通常在创建表的时候一起创建，一张表最多只有一个主键索引，索引列的值不允许有空值。</li>
<li><strong>唯一索引</strong>建立在 UNIQUE 字段上的索引，一张表可以有多个唯一索引，索引列的值必须唯一，但是允许有空值。</li>
<li><strong>普通索引</strong>就是建立在普通字段上的索引，既不要求字段为主键，也不要求字段为 UNIQUE。</li>
<li><strong>前缀索引</strong>是指对字符类型字段的前几个字符建立的索引，而不是在整个字段上建立的索引，前缀索引可以建立在字段类型为 char、 varchar、binary、varbinary 的列上。
<ul>
<li>使用前缀索引的目的是为了减少索引占用的存储空间，提升查询效率。</li>
</ul>
</li>
</ul>
<h4 id="按字段个数分类" tabindex="-1"><a class="header-anchor" href="#按字段个数分类" aria-hidden="true">#</a> 按字段个数分类</h4>
<p>从字段个数的角度来看，索引分为单列索引、联合索引（复合索引）。</p>
<ul>
<li>建立在单列上的索引称为单列索引，比如主键索引；</li>
<li>建立在多列上的索引称为联合索引；</li>
</ul>
<h5 id="联合索引" tabindex="-1"><a class="header-anchor" href="#联合索引" aria-hidden="true">#</a> 联合索引</h5>
<p>通过将多个字段组合成一个索引，该索引就被称为联合索引。比如将商品表中的 product_no 和 name 字段组合成联合索引 <code v-pre>(product_no, name)</code>，创建联合索引的方式如下：</p>
<div class="language-sql line-numbers-mode" data-ext="sql"><pre v-pre class="language-sql"><code><span class="token keyword">CREATE</span> <span class="token keyword">INDEX</span> index_product_no_name <span class="token keyword">ON</span> product<span class="token punctuation">(</span>product_no<span class="token punctuation">,</span> name<span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>联合索引 <code v-pre>(product_no, name)</code> 的 B+Tree 示意图如下：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220712155601299.png" alt="image-20220712155601299" tabindex="0" loading="lazy"><figcaption>image-20220712155601299</figcaption></figure>
<h3 id="⭐️聚簇索引和非聚簇索引" tabindex="-1"><a class="header-anchor" href="#⭐️聚簇索引和非聚簇索引" aria-hidden="true">#</a> ⭐️聚簇索引和非聚簇索引</h3>
<blockquote>
<p>简洁回答</p>
</blockquote>
<ul>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220718134133927.png" alt="image-20220718134133927" tabindex="0" loading="lazy"><figcaption>image-20220718134133927</figcaption></figure>
...
<h3 id="数据库事务" tabindex="-1"><a class="header-anchor" href="#数据库事务" aria-hidden="true">#</a> 数据库事务？</h3>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="@source/Java/eightpart/personal_images/image-20220613143832546.png" alt="image-20220613143832546" tabindex="0" loading="lazy"><figcaption>image-20220613143832546</figcaption></figure>
<p>另外，关系型数据库（例如：<code v-pre>MySQL</code>、<code v-pre>SQL Server</code>、<code v-pre>Oracle</code> 等）事务都有 <strong>ACID</strong> 特性：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220613143851071.png" alt="image-20220613143851071" tabindex="0" loading="lazy"><figcaption>image-20220613143851071</figcaption></figure>
<ol>
...
<p>🌈 这里要额外补充一点：<strong>只有保证了事务的持久性、原子性、隔离性之后，一致性才能得到保障。也就是说 A、I、D 是手段，C 是目的！</strong> 想必大家也和我一样，被 ACID 这个概念被误导了很久!</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220613144017540.png" alt="image-20220613144017540" tabindex="0" loading="lazy"><figcaption>image-20220613144017540</figcaption></figure>
<p>另外，DDIA 也就是 <a href="https://book.douban.com/subject/30329536/" target="_blank" rel="noopener noreferrer">《Designing Data-Intensive Application（数据密集型应用系统设计）》<ExternalLinkIcon/></a></p>
<p>的作者在他的这本书中如是说：</p>
<blockquote>
...
<blockquote>
<p>部分图选自《MySQL是怎样运行的》</p>
</blockquote>
<p>在典型的应用程序中，多个事务并发运行，经常会操作相同的数据来完成各自的任务（多个用户对同一数据进行操作）。并发虽然是必须的，但可能会导致以下的问题。</p>
<ul>
<li><strong>脏读（Dirty read）:</strong> 当一个事务正在访问数据并且对数据进行了修改，而这种修改还没有提交到数据库中，这时另外一个事务也访问了这个数据，然后使用了这个数据。因为这个数据是还没有提交的数据，那么另外一个事务读到的这个数据是“脏数据”，依据“脏数据”所做的操作可能是不正确的。【简而言之：<strong>如果一个事务读到了另一个未提交事务修改过的数据，那就意味着发生了 脏读</strong> 】</li>
</ul>
<figure><img src="@source/Java/eightpart/personal_images/v2-404a11c565a99a9382aa3ed1d8e05e9a_r.jpg" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<figure><img src="@source/Java/eightpart/personal_images/image-20220706223058381.png" alt="image-20220706223058381" tabindex="0" loading="lazy"><figcaption>image-20220706223058381</figcaption></figure>
<ul>
<li><strong>丢失修改（Lost to modify）/ 脏写:</strong>  指在一个事务读取一个数据时，另外一个事务也访问了该数据，那么在第一个事务中修改了这个数据后，第二个事务也修改了这个数据。这样第一个事务内的修改结果就被丢失，因此称为丢失修改。 例如：事务 1 读取某表中的数据 A=20，事务 2 也读取 A=20，事务 1 修改 A=A-1，事务 2 也修改 A=A-1，最终结果  A=19，事务 1 的修改被丢失。【<strong>如果一个事务修改了另一个未提交事务修改过的数据，那就意味着发生了 脏写</strong>】</li>
</ul>
<figure><img src="@source/Java/eightpart/personal_images/v2-0b5257fd270ef7e6e619e2eec293ec37_r.jpg" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<figure><img src="@source/Java/eightpart/personal_images/image-20220706223114814.png" alt="image-20220706223114814" tabindex="0" loading="lazy"><figcaption>image-20220706223114814</figcaption></figure>
<ul>
...
<figure><img src="@source/Java/eightpart/personal_images/v2-f470fb471caacb70acf65bc2a597b0c5_r.jpg" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<figure><img src="@source/Java/eightpart/personal_images/image-20220706222414240.png" alt="image-20220706222414240" tabindex="0" loading="lazy"><figcaption>image-20220706222414240</figcaption></figure>
<ul>
<li><strong>幻读（Phantom read）:</strong> 幻读与不可重复读类似。它发生在一个事务（T1）读取了几行数据，接着另一个并发事务（T2）插入了一些数据时。在随后的查询中，第一个事务（T1）就会发现多了一些原本不存在的记录，就好像发生了幻觉一样，所以称为幻读。</li>
</ul>
<figure><img src="@source/Java/eightpart/personal_images/v2-c698e64414f7cb1428e3025e133d9592_r.jpg" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<figure><img src="@source/Java/eightpart/personal_images/image-20220706222426455.png" alt="image-20220706222426455" tabindex="0" loading="lazy"><figcaption>image-20220706222426455</figcaption></figure>
<p><strong>不可重复读和幻读区别</strong> ：<code v-pre>&lt;u&gt;</code>不可重复读的重点是修改<code v-pre>&lt;/u&gt;</code>比如多次读取一条记录发现其中某些列的值被修改，<code v-pre>&lt;u&gt;</code>幻读的重点在于新增或者删除<code v-pre>&lt;/u&gt;</code>比如多次查询同一条查询语句（DQL）时，记录发现记录增多或减少了。</p>
<h3 id="⭐有哪些事务隔离级别-mysql-的默认隔离级别是什么" tabindex="-1"><a class="header-anchor" href="#⭐有哪些事务隔离级别-mysql-的默认隔离级别是什么" aria-hidden="true">#</a> ⭐有哪些事务隔离级别，MySQL 的默认隔离级别是什么?</h3>
...
<ul>
<li><strong>表级锁：</strong> MySQL 中锁定粒度最大的一种锁，是针<strong>对非索引字段加的锁</strong>，对当前操作的整张表加锁，实现简单，资源消耗也比较少，加锁快，不会出现死锁。其锁定粒度最大，触发锁冲突的概率最高，并发度最低，MyISAM 和 InnoDB 引擎都支持表级锁。</li>
<li><strong>行级锁：</strong> MySQL 中锁定粒度最小的一种锁，是针<strong>对索引字段加的锁</strong>，只针对当前操作的记录进行加锁。 行级锁能大大减少数据库操作的冲突。其加锁粒度最小，并发度高，但加锁的开销也最大，加锁慢，会出现死锁。</li>
</ul>
<h3 id="共享锁和排他锁" tabindex="-1"><a class="header-anchor" href="#共享锁和排他锁" aria-hidden="true">#</a> 共享锁和排他锁</h3>
...
<div class="language-sql line-numbers-mode" data-ext="sql"><pre v-pre class="language-sql"><code><span class="token comment"># 共享锁</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="数据库索引失效有哪些" tabindex="-1"><a class="header-anchor" href="#数据库索引失效有哪些" aria-hidden="true">#</a> 数据库索引失效有哪些</h3>
<p>详细查看文章：<a href="https://xiaolincoding.com/mysql/index/index_lose.html" target="_blank" rel="noopener noreferrer">https://xiaolincoding.com/mysql/index/index_lose.html<ExternalLinkIcon/></a></p>
<ul>
<li>当我们使用左或者左右模糊匹配的时候，也就是 <code v-pre>like %xx</code> 或者 <code v-pre>like %xx%</code>这两种方式都会造成索引失效；</li>
...
<h3 id="数据库sql优化有哪些方法" tabindex="-1"><a class="header-anchor" href="#数据库sql优化有哪些方法" aria-hidden="true">#</a> 数据库sql优化有哪些方法</h3>
<blockquote>
<p>规范</p>
</blockquote>
<p><a href="https://javaguide.cn/database/mysql/mysql-high-performance-optimization-specification-recommendations.html#%E6%95%B0%E6%8D%AE%E5%BA%93%E5%91%BD%E4%BB%A4%E8%A7%84%E8%8C%83" target="_blank" rel="noopener noreferrer">MySQL高性能优化规范建议<ExternalLinkIcon/></a></p>
<ol>
<li>尽量避免使用子查询</li>
<li>用IN来替换OR</li>
...
<p><a href="https://www.pdai.tech/md/db/sql-lan/sql-lan-optimize.html#sql%E8%AF%AD%E8%A8%80---sql%E8%AF%AD%E5%8F%A5%E4%BC%98%E5%8C%96" target="_blank" rel="noopener noreferrer">SQL语句优化<ExternalLinkIcon/></a></p>
...
<h3 id="为什么mysql-没有使用hash表作为索引的数据结构呢" tabindex="-1"><a class="header-anchor" href="#为什么mysql-没有使用hash表作为索引的数据结构呢" aria-hidden="true">#</a> 为什么MySQL 没有使用Hash表作为索引的数据结构呢？</h3>
<p><strong>1.Hash 冲突问题</strong> ：JDK1.8 之前 <code v-pre>HashMap</code> 就是通过链地址法来解决哈希冲突的。不过，JDK1.8 以后 <code v-pre>HashMap</code>为了减少链表过长的时候搜索时间过长引入了红黑树，不过对于数据库来说这还不算最大的缺点。</p>
...
<h3 id="⭐️为什么mysql底层要用b-树-和b树有什么区别" tabindex="-1"><a class="header-anchor" href="#⭐️为什么mysql底层要用b-树-和b树有什么区别" aria-hidden="true">#</a> ⭐️为什么MySQL底层要用B+树？和B树有什么区别？</h3>
<p>图解B+树：<a href="https://zhuanlan.zhihu.com/p/54102723" target="_blank" rel="noopener noreferrer">https://zhuanlan.zhihu.com/p/54102723<ExternalLinkIcon/></a></p>
...
<p>MyISAM 引擎中，B+Tree 叶节点的 data 域存放的是数据记录的地址。在索引检索的时候，首先按照 B+Tree  搜索算法搜索索引，如果指定的 Key 存在，则取出其 data 域的值，然后<strong>以 data  域的值为地址读取相应的数据记录</strong>。这被称为“<strong>非聚簇索引</strong>”。</p>
<p>InnoDB 引擎中，其数据文件本身就是索引文件。相比 MyISAM，索引文件和数据文件是分离的，其表数据文件本身就是按 B+Tree  组织的一个索引结构，<strong>树的叶节点 data 域保存了完整的数据记录</strong>。这个索引的 key 是数据表的主键，因此 InnoDB  表数据文件本身就是主索引。这被称为**“聚簇索引（或聚集索引）**”，而其余的索引都作为辅助索引，辅助索引的 data  域存储相应记录主键的值而不是地址，这也是和 MyISAM 不同的地方。在根据主索引搜索时，直接找到 key  所在的节点即可取出数据；在根据辅助索引查找时，则需要先取出主键的值，再走一遍主索引。  因此，在设计表的时候，不建议使用过长的字段作为主键，也不建议使用非单调的字段作为主键，这样会造成主索引频繁分裂。</p>
<h3 id="详解b-树" tabindex="-1"><a class="header-anchor" href="#详解b-树" aria-hidden="true">#</a> 详解B+树</h3>
<h4 id="什么是-b-树" tabindex="-1"><a class="header-anchor" href="#什么是-b-树" aria-hidden="true">#</a> 什么是 B+ 树？</h4>
<p>B+ 树就是对 B 树做了一个升级，MySQL 中索引的数据结构就是采用了 B+ 树，B+ 树结构如下图：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220718133551150.png" alt="image-20220718133551150" tabindex="0" loading="lazy"><figcaption>image-20220718133551150</figcaption></figure>
...
<h4 id="b-树是如何进行查询的" tabindex="-1"><a class="header-anchor" href="#b-树是如何进行查询的" aria-hidden="true">#</a> B+ 树是如何进行查询的？</h4>
<p>一个数据页中的记录检索，因为一个数据页中的记录是有限的，且主键值是有序的，所以通过对所有记录进行分组，然后将组号（槽号）存储到页目录，使其起到索引作用，通过二分查找的方法快速检索到记录在哪个分组，来降低检索的时间复杂度。</p>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220718134312198.png" alt="image-20220718134312198" tabindex="0" loading="lazy"><figcaption>image-20220718134312198</figcaption></figure>
...
<h4 id="单点查询" tabindex="-1"><a class="header-anchor" href="#单点查询" aria-hidden="true">#</a> 单点查询</h4>
...
<h4 id="插入和删除效率" tabindex="-1"><a class="header-anchor" href="#插入和删除效率" aria-hidden="true">#</a> 插入和删除效率</h4>
...
<figure><img src="https://img-blog.csdnimg.cn/25508b0cd9c44ef6937fdd737020a7f1.gif" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<blockquote>
...
<figure><img src="https://img-blog.csdnimg.cn/2be62679487640bbaac663fa96c7f35f.gif" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>甚至，B+ 树在删除根节点的时候，由于存在冗余的节点，所以不会发生复杂的树的变形，比如下面这个动图是删除 B+ 树根节点的过程：</p>
<figure><img src="https://img-blog.csdnimg.cn/23730b5af987480fabff0f1d142a2b6c.gif" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>B 树则不同，B 树没有冗余节点，删除节点的时候非常复杂，比如删除根节点中的数据，可能涉及复杂的树的变形，比如下面这个动图是删除 B 树根节点的过程：</p>
<figure><img src="https://img-blog.csdnimg.cn/img_convert/7552002f9b8195ab650d431bfe66cce2.gif" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
...
<h4 id="范围查询" tabindex="-1"><a class="header-anchor" href="#范围查询" aria-hidden="true">#</a> 范围查询</h4>
...
<h4 id="mysql-中的-b-树" tabindex="-1"><a class="header-anchor" href="#mysql-中的-b-树" aria-hidden="true">#</a> MySQL 中的 B+ 树</h4>
<p>MySQL 的存储方式根据存储引擎的不同而不同，我们最常用的就是 Innodb 存储引擎，它就是采用了 B+ 树作为了索引的数据结构。</p>
<p>下图就是 Innodb 里的 B+ 树：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220718133843334.png" alt="image-20220718133843334" tabindex="0" loading="lazy"><figcaption>image-20220718133843334</figcaption></figure>
...
<h3 id="mysql三大日志-binlog、redo-log、undo-log" tabindex="-1"><a class="header-anchor" href="#mysql三大日志-binlog、redo-log、undo-log" aria-hidden="true">#</a> MySQL三大日志（binlog、redo log、undo log）</h3>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220613210057735.png" alt="image-20220613210057735" tabindex="0" loading="lazy"><figcaption>image-20220613210057735</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706212928940.png" alt="image-20220706212928940" tabindex="0" loading="lazy"><figcaption>image-20220706212928940</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706212937926.png" alt="image-20220706212937926" tabindex="0" loading="lazy"><figcaption>image-20220706212937926</figcaption></figure>
<blockquote>
...
<h5 id="pagecache" tabindex="-1"><a class="header-anchor" href="#pagecache" aria-hidden="true">#</a> PageCache</h5>
<p>我们知道文件一般存放在硬盘（机械硬盘或固态硬盘）中，CPU 并不能直接访问硬盘中的数据，而是需要先将硬盘中的数据读入到内存中，然后才能被 CPU 访问。</p>
<p>由于读写硬盘的速度比读写内存要慢很多（DDR4 内存读写速度是机械硬盘500倍，是固态硬盘的200倍），所以为了避免每次读写文件时，都需要对硬盘进行读写操作，Linux 内核使用 <code v-pre>页缓存（Page Cache）</code> 机制来对文件中的数据进行缓存。</p>
<blockquote>
<p>本文使用的 Linux 内核版本为：Linux-2.6.23</p>
</blockquote>
<h6 id="什么是页缓存" tabindex="-1"><a class="header-anchor" href="#什么是页缓存" aria-hidden="true">#</a> 什么是页缓存</h6>
<p>为了提升对文件的读写效率，Linux 内核会以页大小（4KB）为单位，将文件划分为多数据块。当用户对文件中的某个数据块进行读写操作时，内核首先会申请一个内存页（称为 <code v-pre>页缓存</code>）与文件中的数据块进行绑定。如下图所示：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220725150519009.png" alt="image-20220725150519009" tabindex="0" loading="lazy"><figcaption>image-20220725150519009</figcaption></figure>
<p>如上图所示，当用户对文件进行读写时，实际上是对文件的 <code v-pre>页缓存</code> 进行读写。所以对文件进行读写操作时，会分以下两种情况进行处理：</p>
<ul>
...
<h6 id="页缓存的实现" tabindex="-1"><a class="header-anchor" href="#页缓存的实现" aria-hidden="true">#</a> 页缓存的实现</h6>
<p>前面主要介绍了页缓存的作用和原理，接下来我们将会分析 Linux 内核是怎么实现页缓存机制的。</p>
<ol>
<li>address_space</li>
</ol>
<p>在 Linux 内核中，使用 <code v-pre>file</code> 对象来描述一个被打开的文件，其中有个名为 <code v-pre>f_mapping</code> 的字段，定义如下：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>从上面代码可以看出，<code v-pre>f_mapping</code> 字段的类型为 <code v-pre>address_space</code> 结构，其定义如下：</p>
<div class="language-c line-numbers-mode" data-ext="c"><pre v-pre class="language-c"><code><span class="token keyword">struct</span> <span class="token class-name">address_space</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">.</span><span class="token punctuation">.</span><span class="token punctuation">.</span>
<span class="token punctuation">}</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>address_space</code> 结构其中的一个作用就是用于存储文件的 <code v-pre>页缓存</code>，下面介绍一下各个字段的作用：</p>
<ul>
<li><code v-pre>host</code>：指向当前 <code v-pre>address_space</code> 对象所属的文件 <code v-pre>inode</code> 对象（每个文件都使用一个 <code v-pre>inode</code> 对象表示）。</li>
<li><code v-pre>page_tree</code>：用于存储当前文件的 <code v-pre>页缓存</code>。</li>
<li><code v-pre>tree_lock</code>：用于防止并发访问 <code v-pre>page_tree</code> 导致的资源竞争问题。</li>
</ul>
<p>从 <code v-pre>address_space</code> 对象的定义可以看出，文件的 <code v-pre>页缓存</code> 使用了 <code v-pre>radix树</code> 来存储。</p>
<blockquote>
<p><code v-pre>radix树</code>：又名基数树，它使用键值（key-value）对的形式来保存数据，并且可以通过键快速查找到其对应的值。内核以文件读写操作中的数据 <code v-pre>偏移量</code> 作为键，以数据偏移量所在的 <code v-pre>页缓存</code> 作为值，存储在 <code v-pre>address_space</code> 结构的 <code v-pre>page_tree</code> 字段中。</p>
</blockquote>
<p>下图展示了上述各个结构之间的关系：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220725150553230.png" alt="image-20220725150553230" tabindex="0" loading="lazy"><figcaption>image-20220725150553230</figcaption></figure>
<p>如果对 <code v-pre>radix树</code> 不太了解，可以简单将其看成可以通过文件偏移量快速找到其所在 <code v-pre>页缓存</code> 的结构，有机会我会另外写一篇关于 <code v-pre>radix树</code> 的文章。</p>
<ol start="2">
<li>读文件操作</li>
</ol>
<p>现在我们来分析一下读取文件数据的过程，用户可以通过调用 <code v-pre>read</code> 系统调用来读取文件中的数据，其调用链如下：</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>read()
└→ sys_read()
   └→ vfs_read()
      └→ do_sync_read()
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>从上面的调用链可以看出，<code v-pre>read</code> 系统调用最终会调用 <code v-pre>do_generic_mapping_read</code> 函数来读取文件中的数据，其实现如下：</p>
<div class="language-c line-numbers-mode" data-ext="c"><pre v-pre class="language-c"><code><span class="token keyword">void</span> <span class="token function">do_generic_mapping_read</span><span class="token punctuation">(</span><span class="token keyword">struct</span> <span class="token class-name">address_space</span> <span class="token operator">*</span>mapping<span class="token punctuation">,</span>
...
out<span class="token operator">:</span>
    <span class="token punctuation">.</span><span class="token punctuation">.</span><span class="token punctuation">.</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>do_generic_mapping_read</code> 函数的实现比较复杂，经过精简后，上面代码只留下最重要的逻辑，可以归纳为以下几个步骤：</p>
<ul>
<li>通过调用 <code v-pre>find_get_page</code> 函数查找要读取的文件偏移量所对应的页缓存是否存在，如果存在就把页缓存中的数据拷贝到应用程序的内存中。</li>
<li>否则调用 <code v-pre>page_cache_alloc_cold</code> 函数申请一个空闲的内存页作为新的页缓存，并且通过调用 <code v-pre>add_to_page_cache_lru</code> 函数把新申请的页缓存添加到文件页缓存和 LRU 队列中（后面会介绍）。</li>
<li>通过调用 <code v-pre>readpage</code> 接口从文件中读取数据到页缓存中，并且把页缓存的数据拷贝到应用程序的内存中。</li>
</ul>
<p>从上面代码可以看出，当页缓存不存在时会申请一块空闲的内存页作为页缓存，并且通过调用 <code v-pre>add_to_page_cache_lru</code> 函数把其添加到文件的页缓存和 LRU 队列中。我们来看看 <code v-pre>add_to_page_cache_lru</code> 函数的实现：</p>
<div class="language-c line-numbers-mode" data-ext="c"><pre v-pre class="language-c"><code> <span class="token keyword">int</span> <span class="token function">add_to_page_cache_lru</span><span class="token punctuation">(</span><span class="token keyword">struct</span> <span class="token class-name">page</span> <span class="token operator">*</span>page<span class="token punctuation">,</span> <span class="token keyword">struct</span> <span class="token class-name">address_space</span> <span class="token operator">*</span>mapping<span class="token punctuation">,</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>add_to_page_cache_lru</code> 函数主要完成两个工作：</p>
<ul>
<li>通过调用 <code v-pre>add_to_page_cache</code> 函数把页缓存添加到文件页缓存中，也就是添加到 <code v-pre>address_space</code> 结构的 <code v-pre>page_tree</code> 字段中。</li>
<li>通过调用 <code v-pre>lru_cache_add</code> 函数把页缓存添加到 LRU 队列中。LRU 队列用于当系统内存不足时，对页缓存进行清理时使用。</li>
</ul>
<h6 id="总结" tabindex="-1"><a class="header-anchor" href="#总结" aria-hidden="true">#</a> 总结</h6>
<p>本文主要介绍了 <code v-pre>页缓存</code> 的作用和原理，并且介绍了在读取文件数据时对页缓存的处理过程。本文并没有介绍写文件操作对应的页缓存处理和当系统内存不足时怎么释放页缓存，有兴趣的话可以自行阅读相关的代码实现。</p>
<h5 id="刷盘时机" tabindex="-1"><a class="header-anchor" href="#刷盘时机" aria-hidden="true">#</a> 刷盘时机</h5>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213001613.png" alt="image-20220706213001613" tabindex="0" loading="lazy"><figcaption>image-20220706213001613</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213009977.png" alt="image-20220706213009977" tabindex="0" loading="lazy"><figcaption>image-20220706213009977</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213032809.png" alt="image-20220706213032809" tabindex="0" loading="lazy"><figcaption>image-20220706213032809</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213045311.png" alt="image-20220706213045311" tabindex="0" loading="lazy"><figcaption>image-20220706213045311</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213100010.png" alt="image-20220706213100010" tabindex="0" loading="lazy"><figcaption>image-20220706213100010</figcaption></figure>
...
<h5 id="日志文件组" tabindex="-1"><a class="header-anchor" href="#日志文件组" aria-hidden="true">#</a> 日志文件组</h5>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213128010.png" alt="image-20220706213128010" tabindex="0" loading="lazy"><figcaption>image-20220706213128010</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213147439.png" alt="image-20220706213147439" tabindex="0" loading="lazy"><figcaption>image-20220706213147439</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213155597.png" alt="image-20220706213155597" tabindex="0" loading="lazy"><figcaption>image-20220706213155597</figcaption></figure>
<h5 id="redo-log-小结" tabindex="-1"><a class="header-anchor" href="#redo-log-小结" aria-hidden="true">#</a> redo log 小结</h5>
...
<p>如果是写 <code v-pre>redo log</code>，一行记录可能就占几十 <code v-pre>Byte</code>，只包含表空间号、数据页号、磁盘文件偏移 量、更新值，再加上是顺序写，所以刷盘速度很快。</p>
...
<h4 id="binlog" tabindex="-1"><a class="header-anchor" href="#binlog" aria-hidden="true">#</a> binlog</h4>
...
<h5 id="记录格式" tabindex="-1"><a class="header-anchor" href="#记录格式" aria-hidden="true">#</a> 记录格式</h5>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213326407.png" alt="image-20220706213326407" tabindex="0" loading="lazy"><figcaption>image-20220706213326407</figcaption></figure>
<p>同步数据时，会执行记录的 <code v-pre>SQL</code>语句，但是</p>
<p>有个问题，<code v-pre>update_time=now()</code>这里会获取当前系统时间，直接执行会导致与原库的数据不一致。</p>
<p>为了解决这种问题，我们需要指定为 <code v-pre>row</code>，记录的内容不再是简单的 <code v-pre>SQL</code>语句了，还包含操作的具体数据，记录内容如下</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213333425.png" alt="image-20220706213333425" tabindex="0" loading="lazy"><figcaption>image-20220706213333425</figcaption></figure>
...
<h5 id="写入机制" tabindex="-1"><a class="header-anchor" href="#写入机制" aria-hidden="true">#</a> 写入机制</h5>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213409674.png" alt="image-20220706213409674" tabindex="0" loading="lazy"><figcaption>image-20220706213409674</figcaption></figure>
<ul>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213418556.png" alt="image-20220706213418556" tabindex="0" loading="lazy"><figcaption>image-20220706213418556</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213426711.png" alt="image-20220706213426711" tabindex="0" loading="lazy"><figcaption>image-20220706213426711</figcaption></figure>
...
<h4 id="两阶段提交" tabindex="-1"><a class="header-anchor" href="#两阶段提交" aria-hidden="true">#</a> 两阶段提交</h4>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213448142.png" alt="image-20220706213448142" tabindex="0" loading="lazy"><figcaption>image-20220706213448142</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213523943.png" alt="image-20220706213523943" tabindex="0" loading="lazy"><figcaption>image-20220706213523943</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213531140.png" alt="image-20220706213531140" tabindex="0" loading="lazy"><figcaption>image-20220706213531140</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213538608.png" alt="image-20220706213538608" tabindex="0" loading="lazy"><figcaption>image-20220706213538608</figcaption></figure>
<p>使用<strong>两阶段提交</strong>后，写入 <code v-pre>binlog</code>时发生异常也不会有影响，因为 <code v-pre>MySQL</code>根据 <code v-pre>redo log</code>日志恢复数据时，发现 <code v-pre>redo log</code>还处于 <code v-pre>prepare</code>阶段，并且没有对应 <code v-pre>binlog</code>日志，就会回滚该事务。<img src="@source/Java/eightpart/personal_images/image-20220706213548852.png" alt="image-20220706213548852" loading="lazy"></p>
<p>再看一个场景，<code v-pre>redo log</code>设置 <code v-pre>commit</code>阶段发生异常，那会不会回滚事务呢？</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213556183.png" alt="image-20220706213556183" tabindex="0" loading="lazy"><figcaption>image-20220706213556183</figcaption></figure>
...
<p>执行一条语句是否自动提交事务，是由 <code v-pre>autocommit</code> 参数决定的，默认是开启。所以，执行一条 update 语句也是会使用事务的。</p>
<p>那么，考虑一个问题。一个事务在执行过程中，在还没有提交事务之前，如果MySQL 发生了崩溃，要怎么回滚到事务之前的数据呢？</p>
<p>如果我们每次在事务执行过程中，都记录下回滚时需要的信息到一个日志里，那么在事务执行中途发生了 MySQL 崩溃后，就不用担心无法回滚到事务之前的数据，我们可以通过这个日志回滚到事务之前的数据。</p>
<p>实现这一机制就是  <strong>undo log（回滚日志），它保证了事务的 ACID 特性 (opens new window)中的原子性（Atomicity）。</strong></p>
<p>undo log 是一种用于撤销回退的日志。在事务没提交之前，MySQL 会先记录更新前的数据到 undo log 日志文件里面，当事务回滚时，可以利用 undo log 来进行回滚。如下图：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213728255.png" alt="image-20220706213728255" tabindex="0" loading="lazy"><figcaption>image-20220706213728255</figcaption></figure>
...
<p>版本链如下图：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220706213736248.png" alt="image-20220706213736248" tabindex="0" loading="lazy"><figcaption>image-20220706213736248</figcaption></figure>
...
<p>这两个隔离级别实现是通过「事务的 Read View 里的字段」和「记录中的两个隐藏列（trx_id 和 roll_pointer）」的比对，如果不满足可见行，就会顺着  undo log 版本链里找到满足其可见性的记录，从而控制并发事务访问同一个记录时的行为，这就叫  MVCC（多版本并发控制）。具体的实现可以看：<a href="https://xiaolincoding.com/mysql/transaction/mvcc.html#%E4%BA%8B%E5%8A%A1%E7%9A%84%E9%9A%94%E7%A6%BB%E7%BA%A7%E5%88%AB%E6%9C%89%E5%93%AA%E4%BA%9B" target="_blank" rel="noopener noreferrer">事务隔离级别是怎么实现的<ExternalLinkIcon/></a>？ 因此，undo log 两大作用：</p>
<ul>
...
<h3 id="图解mvcc" tabindex="-1"><a class="header-anchor" href="#图解mvcc" aria-hidden="true">#</a> 图解MVCC</h3>
<p>纲要：</p>
<ul>
<li>当前读和快照读的概念</li>
<li>事务隔离级别</li>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220706231641283.png" alt="image-20220706231641283" tabindex="0" loading="lazy"><figcaption>image-20220706231641283</figcaption></figure>
<ul>
<li>当理解了上述概念可以对此例子进行加深理解</li>
</ul>
<figure><img src="@source/Java/eightpart/personal_images/image-20220706220605710.png" alt="image-20220706220605710" tabindex="0" loading="lazy"><figcaption>image-20220706220605710</figcaption></figure>
<h3 id="innodb存储引擎对mvcc的实现" tabindex="-1"><a class="header-anchor" href="#innodb存储引擎对mvcc的实现" aria-hidden="true">#</a> InnoDB存储引擎对MVCC的实现</h3>
...
<h4 id="readview" tabindex="-1"><a class="header-anchor" href="#readview" aria-hidden="true">#</a> ReadView</h4>
...
<p><strong>事务可见性示意图</strong></p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220711215228405.png" alt="image-20220711215228405" tabindex="0" loading="lazy"><figcaption>image-20220711215228405</figcaption></figure>
<h4 id="undo-log-1" tabindex="-1"><a class="header-anchor" href="#undo-log-1" aria-hidden="true">#</a> undo-log</h4>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220711215325716.png" alt="image-20220711215325716" tabindex="0" loading="lazy"><figcaption>image-20220711215325716</figcaption></figure>
<ol start="2">
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220711215339796.png" alt="image-20220711215339796" tabindex="0" loading="lazy"><figcaption>image-20220711215339796</figcaption></figure>
<p><strong>数据第二次被修改时：</strong></p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220711215345905.png" alt="image-20220711215345905" tabindex="0" loading="lazy"><figcaption>image-20220711215345905</figcaption></figure>
<p>不同事务或者相同事务的对同一记录行的修改，会使该记录行的 <code v-pre>undo log</code> 成为一条链表，链首就是最新的记录，链尾就是最早的旧记录</p>
<h4 id="数据可见性算法" tabindex="-1"><a class="header-anchor" href="#数据可见性算法" aria-hidden="true">#</a> 数据可见性算法</h4>
...
<p>具体的比较算法如下</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220711215410118.png" alt="image-20220711215410118" tabindex="0" loading="lazy"><figcaption>image-20220711215410118</figcaption></figure>
<ol>
...
<li>如果 m_up_limit_id &lt;= DB_TRX_ID &lt;  m_low_limit_id，表明最新修改该行的事务（DB_TRX_ID）在当前事务创建快照的时候可能处于“活动状态”或者“已提交状态”；所以就要对活跃事务列表 m_ids 进行查找（源码中是用的二分查找，因为是有序的）
<ul>
...
<li>在该记录行的 DB_ROLL_PTR 指针所指向的 <code v-pre>undo log</code> 取出快照记录，用快照记录的 DB_TRX_ID 跳到步骤 1 重新开始判断，直到找到满足的快照版本或返回空</li>
</ol>
<h3 id="mysql有几种高可用方案" tabindex="-1"><a class="header-anchor" href="#mysql有几种高可用方案" aria-hidden="true">#</a> MySQL有几种高可用方案</h3>
<ul>
<li>主从或主主半同步复制</li>
<li>半同步复制优化</li>
<li>高可用架构优化</li>
...
<h4 id="主从或主主半同步复制" tabindex="-1"><a class="header-anchor" href="#主从或主主半同步复制" aria-hidden="true">#</a> 主从或主主半同步复制</h4>
<p>用双节点数据库，搭建单向或者双向的半同步复制。架构如下：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220801004733873.png" alt="image-20220801004733873" tabindex="0" loading="lazy"><figcaption>image-20220801004733873</figcaption></figure>
...
<h4 id="半同步复制优化" tabindex="-1"><a class="header-anchor" href="#半同步复制优化" aria-hidden="true">#</a> 半同步复制优化</h4>
<p>半同步复制机制是可靠的，可以保证数据一致性的。但是如果网络发生波动，半同步复制发生超时会切换为异步复制，异复制是无法保证数据的一致性的。因此，可以在半同复制的基础上优化一下，尽可能保证半同复制。如<strong>双通道复制</strong>方案</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220801004755753.png" alt="image-20220801004755753" tabindex="0" loading="lazy"><figcaption>image-20220801004755753</figcaption></figure>
<ul>
<li>优点：这种方案架构、部署也比较简单，主机宕机也是直接切换即可。比方案1的半同步复制，更能保证数据的一致性。</li>
<li>缺点：需要修改内核源码或者使用mysql通信协议，没有从根本上解决数据一致性问题</li>
</ul>
<h4 id="高可用架构优化" tabindex="-1"><a class="header-anchor" href="#高可用架构优化" aria-hidden="true">#</a> 高可用架构优化</h4>
<p>保证高可用，可以把主从双节点数据库扩展为数据库集群。Zookeeper可以作为集群管理，它使用分布式算法保证集群数据的一致性，可以较好的避免网络分区现象的产生。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220801004842095.png" alt="image-20220801004842095" tabindex="0" loading="lazy"><figcaption>image-20220801004842095</figcaption></figure>
<ul>
<li>优点：保证了整个系统的高可用性，扩展性也较好，可以扩展为大规模集群。</li>
<li>缺点：数据一致性<strong>仍然依赖于原生的mysql半同步复制</strong>；引入Zookeeper使系统逻辑更复杂。</li>
</ul>
<h4 id="共享存储" tabindex="-1"><a class="header-anchor" href="#共享存储" aria-hidden="true">#</a> 共享存储</h4>
<blockquote>
<p>共享存储实现了数据库服务器和存储设备的解耦，不同数据库之间的数据同步不再依赖于MySQL的原生复制功能，而是通过磁盘数据同步的手段，来保证数据的一致性。</p>
</blockquote>
<h5 id="drbd磁盘复制" tabindex="-1"><a class="header-anchor" href="#drbd磁盘复制" aria-hidden="true">#</a> DRBD磁盘复制</h5>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220801004923275.png" alt="image-20220801004923275" tabindex="0" loading="lazy"><figcaption>image-20220801004923275</figcaption></figure>
<p>当本地主机出现问题，远程主机上还保留着一份相同的数据，即可以继续使用，保证了数据的安全。</p>
<ul>
<li>优点：部署简单，价格合适，保证数据的强一致性</li>
<li>缺点：对IO性能影响较大，从库不提供读操作</li>
</ul>
<h4 id="分布式协议" tabindex="-1"><a class="header-anchor" href="#分布式协议" aria-hidden="true">#</a> 分布式协议</h4>
<p>分布式协议可以很好解决数据一致性问题。常见的部署方案就是<strong>MySQL cluster</strong>，它是官方集群的部署方案，通过使用NDB存储引擎实时备份冗余数据，实现数据库的高可用性和数据一致性。如下：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220801004942686.png" alt="image-20220801004942686" tabindex="0" loading="lazy"><figcaption>image-20220801004942686</figcaption></figure>
<ul>
<li>优点：不依赖于第三方软件，可以实现数据的强一致性；</li>
<li>缺点：配置较复杂；需要使用NDB储存引擎；至少三节点；</li>
</ul>
<h3 id="读写分离了解吗" tabindex="-1"><a class="header-anchor" href="#读写分离了解吗" aria-hidden="true">#</a> 读写分离了解吗？</h3>
<p>见名思意，根据读写分离的名字，我们就可以知道：<strong>读写分离主要是为了将对数据库的读写操作分散到不同的数据库节点上。</strong> 这样的话，就能够小幅提升写性能，大幅提升读性能。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220617163230641.png" alt="image-20220617163230641" tabindex="0" loading="lazy"><figcaption>image-20220617163230641</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220617163247933.png" alt="image-20220617163247933" tabindex="0" loading="lazy"><figcaption>image-20220617163247933</figcaption></figure>
...
<h3 id="聊聊分库分表-分表为什么要停服这种操作-如果不停服可以怎么做" tabindex="-1"><a class="header-anchor" href="#聊聊分库分表-分表为什么要停服这种操作-如果不停服可以怎么做" aria-hidden="true">#</a> 聊聊分库分表,分表为什么要停服这种操作，如果不停服可以怎么做</h3>
<h4 id="分库分表方案" tabindex="-1"><a class="header-anchor" href="#分库分表方案" aria-hidden="true">#</a> 分库分表方案</h4>
<ul>
...
<h4 id="常用的分库分表中间件" tabindex="-1"><a class="header-anchor" href="#常用的分库分表中间件" aria-hidden="true">#</a> 常用的分库分表中间件</h4>
<ul>
...
<h4 id="分库分表可能遇到的问题" tabindex="-1"><a class="header-anchor" href="#分库分表可能遇到的问题" aria-hidden="true">#</a> 分库分表可能遇到的问题</h4>
<ul>
...
<h4 id="分表要停服嘛-不停服怎么做" tabindex="-1"><a class="header-anchor" href="#分表要停服嘛-不停服怎么做" aria-hidden="true">#</a> 分表要停服嘛？不停服怎么做？</h4>
<p>不用。不停服的时候，应该怎么做呢，分五个步骤：</p>
<ol>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220617163123694.png" alt="image-20220617163123694" tabindex="0" loading="lazy"><figcaption>image-20220617163123694</figcaption></figure>
<ol>
...
<h3 id="数据库如何实现分布式锁" tabindex="-1"><a class="header-anchor" href="#数据库如何实现分布式锁" aria-hidden="true">#</a> 数据库如何实现分布式锁</h3>
<h4 id="基于数据库表-锁表-很少使用" tabindex="-1"><a class="header-anchor" href="#基于数据库表-锁表-很少使用" aria-hidden="true">#</a> 基于数据库表（锁表，很少使用）</h4>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h4 id="基于悲观锁" tabindex="-1"><a class="header-anchor" href="#基于悲观锁" aria-hidden="true">#</a> 基于悲观锁</h4>
...
<h4 id="基于乐观锁" tabindex="-1"><a class="header-anchor" href="#基于乐观锁" aria-hidden="true">#</a> 基于乐观锁</h4>
