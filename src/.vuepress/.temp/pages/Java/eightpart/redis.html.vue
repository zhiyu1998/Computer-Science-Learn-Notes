<template><div><h2 id="♨️-redis" tabindex="-1"><a class="header-anchor" href="#♨️-redis" aria-hidden="true">#</a> ♨️ redis</h2>
<h3 id="说一下redis" tabindex="-1"><a class="header-anchor" href="#说一下redis" aria-hidden="true">#</a> 说一下redis</h3>
...
<h3 id="redis为什么这么快" tabindex="-1"><a class="header-anchor" href="#redis为什么这么快" aria-hidden="true">#</a> Redis为什么这么快？</h3>
<ul>
<li>redis完全<strong>基于内存</strong>,绝大部分请求是纯粹的内存操作,非常快速.</li>
...
<li>使用了<strong>多路IO复用模型</strong>,非阻塞IO</li>
<li>使用<strong>底层模型不同</strong>，Redis直接自己构建了 VM (虚拟内存)机制 ，因为一般的系统调用系统函数的话，会浪费一定的时间去移动和请求</li>
</ul>
<h3 id="为什么要用-redis-为什么要用缓存-为什么用-redis-作为-mysql-的缓存" tabindex="-1"><a class="header-anchor" href="#为什么要用-redis-为什么要用缓存-为什么用-redis-作为-mysql-的缓存" aria-hidden="true">#</a> 为什么要用 Redis / 为什么要用缓存？/ 为什么用 Redis 作为 MySQL 的缓存？</h3>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220714213332696.png" alt="image-20220714213332696" tabindex="0" loading="lazy"><figcaption>image-20220714213332696</figcaption></figure>
...
<h3 id="redis-如何实现服务高可用" tabindex="-1"><a class="header-anchor" href="#redis-如何实现服务高可用" aria-hidden="true">#</a> Redis 如何实现服务高可用？</h3>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220715161214356.png" alt="image-20220715161214356" tabindex="0" loading="lazy"><figcaption>image-20220715161214356</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220715161234139.png" alt="image-20220715161234139" tabindex="0" loading="lazy"><figcaption>image-20220715161234139</figcaption></figure>
<blockquote>
<p>切片集群模式</p>
</blockquote>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220715161306957.png" alt="image-20220715161306957" tabindex="0" loading="lazy"><figcaption>image-20220715161306957</figcaption></figure>
...
<h3 id="redis-和-memcached-有什么区别" tabindex="-1"><a class="header-anchor" href="#redis-和-memcached-有什么区别" aria-hidden="true">#</a> Redis 和 Memcached 有什么区别？</h3>
...
<p><strong>分布式锁</strong> ： 通过 Redis 来做分布式锁是一种比较常见的方式。通常情况下，我们都是基于 Redisson 来实现分布式锁。相关阅读：<a href="https://mp.weixin.qq.com/s/CbnPRfvq4m1sqo2uKI6qQw" target="_blank" rel="noopener noreferrer">《分布式锁中的王者方案 - Redisson》<ExternalLinkIcon/></a></p>
<p><strong>限流</strong> ：一般是通过 Redis + Lua 脚本的方式来实现限流。相关阅读：<a href="https://mp.weixin.qq.com/s/kyFAWH3mVNJvurQDt4vchA" target="_blank" rel="noopener noreferrer">《我司用了 6 年的 Redis 分布式限流器，可以说是非常厉害了！》<ExternalLinkIcon/></a></p>
...
<p>......</p>
<h3 id="⭐基本数据类型、应用场景" tabindex="-1"><a class="header-anchor" href="#⭐基本数据类型、应用场景" aria-hidden="true">#</a> ⭐基本数据类型、应用场景</h3>
<blockquote>
<p>5种基本数据类型：string、list、hash、set、zset</p>
<p>推荐阅读：<a href="https://mp.weixin.qq.com/s/qptE172slg_6Tl1yuzdbfw" target="_blank" rel="noopener noreferrer">图解 Redis 数据结构<ExternalLinkIcon/></a></p>
</blockquote>
<h4 id="_1️⃣string" tabindex="-1"><a class="header-anchor" href="#_1️⃣string" aria-hidden="true">#</a> 1️⃣string</h4>
<ol>
...
<li><strong>应用场景：</strong> 一般常用在需要<strong>计数</strong>的场景，比如用户的访问次数、热点文章的点赞转发数量；<strong>缓存</strong>，经典使用场景，把常用信息，字符串，图片或者视频等信息放到redis中，redis作为缓存层，mysql做持久化层，降低mysql的读写压力；<strong>session</strong>，常见方案spring session + redis实现session共享等等。</li>
</ol>
<h4 id="_2️⃣list" tabindex="-1"><a class="header-anchor" href="#_2️⃣list" aria-hidden="true">#</a> 2️⃣list</h4>
<ol>
...
<li><strong>应用场景:</strong> 发布与订阅或者说消息队列、慢查询。
<ol>
<li><strong>微博TimeLine</strong>: 有人发布微博，用lpush加入时间轴，展示新的列表信息。</li>
</ol>
</li>
</ol>
<h4 id="_3️⃣hash" tabindex="-1"><a class="header-anchor" href="#_3️⃣hash" aria-hidden="true">#</a> 3️⃣hash</h4>
<ol>
...
<li><strong>应用场景:</strong> 系统中对象数据的存储，<strong>缓存</strong>： 能直观，相比string更节省空间，的维护缓存信息，如用户信息，视频信息等。</li>
</ol>
<h4 id="_4️⃣set" tabindex="-1"><a class="header-anchor" href="#_4️⃣set" aria-hidden="true">#</a> 4️⃣set</h4>
<ol>
...
<li><strong>应用场景:</strong> 需要存放的数据不能重复以及需要获取多个数据源交集和并集等场景；<strong>标签</strong>（tag）,给用户添加标签，或者用户给消息添加标签，这样有同一标签或者类似标签的可以给推荐关注的事或者关注的人；<strong>点赞，或点踩，收藏等</strong>，可以放到set中实现。</li>
</ol>
<h4 id="_5️⃣zset" tabindex="-1"><a class="header-anchor" href="#_5️⃣zset" aria-hidden="true">#</a> 5️⃣Zset</h4>
<ol>
<li>
<p><strong>介绍 ：</strong> 和 set 相比，sorted set 增加了一个权重参数 score，使得集合中的元素能够按 score 进行有序排列，还可以通过 score 的范围来获取元素的列表。有点像是 Java 中 HashMap 和 TreeSet 的结合体。</p>
</li>
<li>
<p><strong>常用命令：</strong> <code v-pre>ZADD 将一个带有给定分值的成员添加到有序集合里面,ZRANGE 根据元素在有序集合中所处的位置，从有序集合中获取多个元素,ZREM 如果给定元素成员存在于有序集合中，那么就移除这个元素</code> 等。</p>
</li>
<li>
<p><strong>应用场景:</strong> <strong>排行榜</strong>   需要对数据根据某个权重进行排序的场景。例如小说视频等网站需要对用户上传的小说视频做排行榜，榜单可以按照用户关注数，更新时间，字数等打分，做排行；在直播系统中，实时排行信息包含直播间在线用户列表；各种礼物<strong>排行榜</strong>，弹幕消息（可以理解为按消息维度的消息排行榜）等信息。</p>
</li>
<li>
<p>有序集合是通过两种数据结构实现：</p>
<ol>
...
<h4 id="特殊类型-bitmap" tabindex="-1"><a class="header-anchor" href="#特殊类型-bitmap" aria-hidden="true">#</a> 特殊类型： bitmap</h4>
<ol>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220716153002622.png" alt="image-20220716153002622" tabindex="0" loading="lazy"><figcaption>image-20220716153002622</figcaption></figure>
<p>图中的蓝色部分是一个事件循环，是由主线程负责的，可以看到网络 I/O 和命令处理都是单线程。 Redis 初始化的时候，会做下面这几年事情：</p>
<ul>
<li>首先，调用 epoll_create() 创建一个 epoll 对象和调用 socket() 一个服务端 socket</li>
<li>然后，调用 bind() 绑定端口和调用 listen() 监听该 socket；</li>
<li>然后，将调用 epoll_crt() 将 listen socket 加入到 epoll，同时注册「连接事件」处理函数。</li>
</ul>
...
<blockquote>
<p>⚠️ 提醒：篇幅过长，如果没有耐心建议直接背诵即可</p>
<p><a href="https://xiaolincoding.com/os/8_network_system/reactor.html#%E5%8D%95-reactor-%E5%8D%95%E8%BF%9B%E7%A8%8B-%E7%BA%BF%E7%A8%8B" target="_blank" rel="noopener noreferrer">深入了解Reactor直通车<ExternalLinkIcon/></a></p>
<p><a href="https://mp.weixin.qq.com/s/Qpa0qXxuIM8jrBqDaXmVNA" target="_blank" rel="noopener noreferrer">I/O多路复用直通车<ExternalLinkIcon/></a></p>
</blockquote>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="redis-是如何判断数据是否过期的呢" tabindex="-1"><a class="header-anchor" href="#redis-是如何判断数据是否过期的呢" aria-hidden="true">#</a> Redis 是如何判断数据是否过期的呢？</h3>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>过期字典数据结构结构如下：</p>
<ul>
<li>过期字典的 key 是一个指针，指向某个键对象；</li>
<li>过期字典的 value 是一个 long long 类型的整数，这个整数保存了 key 的过期时间；</li>
</ul>
<p>过期字典的数据结构如下图所示：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220714211943265.png" alt="image-20220714211943265" tabindex="0" loading="lazy"><figcaption>image-20220714211943265</figcaption></figure>
...
<p>过期键判断流程如下图所示：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220714211925160.png" alt="image-20220714211925160" tabindex="0" loading="lazy"><figcaption>image-20220714211925160</figcaption></figure>
<blockquote>
<p>拓展：大量 key 集中过期问题</p>
</blockquote>
<p>因为导致客户端请求没办法被及时处理，响应速度会比较慢。</p>
<p>如何解决呢？下面是两种常见的方法：</p>
<ol>
<li>给 key 设置<strong>随机过期时间</strong>。</li>
<li>开启 lazy-free（<strong>惰性删除</strong>/延迟释放） 。lazy-free 特性是 Redis 4.0 开始引入的，指的是让 Redis 采用异步方式延迟释放 key 使用的内存，将该操作交给单独的子线程处理，避免阻塞主线程。</li>
</ol>
<p>个人建议不管是否开启 lazy-free，我们都尽量给 key 设置随机过期时间。</p>
<h3 id="过期的数据的删除策略了解么" tabindex="-1"><a class="header-anchor" href="#过期的数据的删除策略了解么" aria-hidden="true">#</a> 过期的数据的删除策略了解么？</h3>
<p>常用的过期数据的删除策略就前两个（重要！自己造缓存轮子的时候需要格外考虑的东西）：</p>
<blockquote>
<p>惰性删除</p>
</blockquote>
...
<h4 id="redis-内存满了-会发生什么" tabindex="-1"><a class="header-anchor" href="#redis-内存满了-会发生什么" aria-hidden="true">#</a> Redis 内存满了，会发生什么？</h4>
...
<p>LFU 全称是 Least Frequently Used 翻译为**最近最不常用的，**LFU 算法是根据数据访问次数来淘汰数据的，它的核心思想是“如果数据过去被访问多次，那么将来被访问的频率也更高”。</p>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220714211622755.png" alt="image-20220714211622755" tabindex="0" loading="lazy"><figcaption>image-20220714211622755</figcaption></figure>
<ul>
...
<ul>
<li><code v-pre>lfu-decay-time</code> 用于调整 logc 的衰减速度，它是一个以分钟为单位的数值，默认值为1，lfu-decay-time 值越大，衰减越慢；</li>
<li><code v-pre>lfu-log-factor</code> 用于调整 logc 的增长速度，lfu-log-factor 值越大，logc 增长越慢。</li>
</ul>
...
<p>跳跃表（skiplist）是一种随机化的数据结构，由 <strong>William Pugh</strong> 在论文《Skip lists: a probabilistic alternative to balanced trees》中提出，是一种可以与平衡树媲美的层次化链表结构——查找、删除、添加等操作都可以在对数期望时间下完成，以下是一个典型的跳跃表例子：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220715150511611.png" alt="image-20220715150511611" tabindex="0" loading="lazy"><figcaption>image-20220715150511611</figcaption></figure>
...
<h4 id="为什么使用跳跃表" tabindex="-1"><a class="header-anchor" href="#为什么使用跳跃表" aria-hidden="true">#</a> 为什么使用跳跃表</h4>
...
<h4 id="本质是解决查找问题" tabindex="-1"><a class="header-anchor" href="#本质是解决查找问题" aria-hidden="true">#</a> 本质是解决查找问题</h4>
<p>我们先来看一个普通的链表结构：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220715150536830.png" alt="image-20220715150536830" tabindex="0" loading="lazy"><figcaption>image-20220715150536830</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220715150547405.png" alt="image-20220715150547405" tabindex="0" loading="lazy"><figcaption>image-20220715150547405</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220715150600549.png" alt="image-20220715150600549" tabindex="0" loading="lazy"><figcaption>image-20220715150600549</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220715150622214.png" alt="image-20220715150622214" tabindex="0" loading="lazy"><figcaption>image-20220715150622214</figcaption></figure>
...
<h4 id="更进一步的跳跃表" tabindex="-1"><a class="header-anchor" href="#更进一步的跳跃表" aria-hidden="true">#</a> 更进一步的跳跃表</h4>
<p><strong>跳跃表 skiplist</strong> 就是受到这种多层链表结构的启发而设计出来的。按照上面生成链表的方式，上面每一层链表的节点个数，是下面一层的节点个数的一半，这样查找过程就非常类似于一个二分查找，使得查找的时间复杂度可以降低到 <em>O(logn)</em>。</p>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220715150657023.png" alt="image-20220715150657023" tabindex="0" loading="lazy"><figcaption>image-20220715150657023</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220715150710832.png" alt="image-20220715150710832" tabindex="0" loading="lazy"><figcaption>image-20220715150710832</figcaption></figure>
<h4 id="跳跃表的实现" tabindex="-1"><a class="header-anchor" href="#跳跃表的实现" aria-hidden="true">#</a> 跳跃表的实现</h4>
<p>Redis 中的跳跃表由 <code v-pre>server.h/zskiplistNode</code> 和 <code v-pre>server.h/zskiplist</code> 两个结构定义，前者为跳跃表节点，后者则保存了跳跃节点的相关信息，同之前的 <code v-pre>集合 list</code> 结构类似，其实只有 <code v-pre>zskiplistNode</code> 就可以实现了，但是引入后者是为了更加方便的操作：</p>
<div class="language-c line-numbers-mode" data-ext="c"><pre v-pre class="language-c"><code><span class="token comment">/* ZSETs use a specialized version of Skiplists */</span>
...
<span class="token keyword">typedef</span> <span class="token keyword">struct</span> <span class="token class-name">zskiplist</span> <span class="token punctuation">{</span>
    <span class="token comment">// 跳跃表头指针</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>正如文章开头画出来的那张标准的跳跃表那样。</p>
<h5 id="随机层数" tabindex="-1"><a class="header-anchor" href="#随机层数" aria-hidden="true">#</a> 随机层数</h5>
<p>对于每一个新插入的节点，都需要调用一个随机算法给它分配一个合理的层数，源码在 <code v-pre>t_zset.c/zslRandomLevel(void)</code> 中被定义：</p>
<div class="language-c line-numbers-mode" data-ext="c"><pre v-pre class="language-c"><code><span class="token keyword">int</span> <span class="token function">zslRandomLevel</span><span class="token punctuation">(</span><span class="token keyword">void</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>直观上期望的目标是 50% 的概率被分配到 <code v-pre>Level 1</code>，25% 的概率被分配到 <code v-pre>Level 2</code>，12.5% 的概率被分配到 <code v-pre>Level 3</code>，以此类推...有 2-63 的概率被分配到最顶层，因为这里每一层的晋升率都是 50%。</p>
<p><strong>Redis 跳跃表默认允许最大的层数是 32</strong>，被源码中 <code v-pre>ZSKIPLIST_MAXLEVEL</code> 定义，当 <code v-pre>Level[0]</code> 有 264 个元素时，才能达到 32 层，所以定义 32 完全够用了。</p>
<h5 id="创建跳跃表" tabindex="-1"><a class="header-anchor" href="#创建跳跃表" aria-hidden="true">#</a> 创建跳跃表</h5>
<p>这个过程比较简单，在源码中的 <code v-pre>t_zset.c/zslCreate</code> 中被定义：</p>
<div class="language-c line-numbers-mode" data-ext="c"><pre v-pre class="language-c"><code>zskiplist <span class="token operator">*</span><span class="token function">zslCreate</span><span class="token punctuation">(</span><span class="token keyword">void</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token comment">// 初始化长度为 0</span>
    ...
    <span class="token comment">// 创建一个层数为 32，分数为 0，没有 value 值的跳跃表头节点</span>
    ...
    <span class="token comment">// 跳跃表头节点初始化</span>
    <span class="token keyword">for</span> <span class="token punctuation">(</span>j <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span> j <span class="token operator">&lt;</span> ZSKIPLIST_MAXLEVEL<span class="token punctuation">;</span> j<span class="token operator">++</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token comment">// 将跳跃表头节点的所有前进指针 forward 设置为 NULL</span>
        ...
        <span class="token comment">// 将跳跃表头节点的所有跨度 span 设置为 0</span>
    ...
    <span class="token punctuation">}</span>
    <span class="token comment">// 跳跃表头节点的后退指针 backward 置为 NULL</span>
    ...
    <span class="token comment">// 表头指向跳跃表尾节点的指针置为 NULL</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>即执行完之后创建了如下结构的初始化跳跃表：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220715150812789.png" alt="image-20220715150812789" tabindex="0" loading="lazy"><figcaption>image-20220715150812789</figcaption></figure>
<h5 id="插入节点实现" tabindex="-1"><a class="header-anchor" href="#插入节点实现" aria-hidden="true">#</a> 插入节点实现</h5>
<p>这几乎是最重要的一段代码了，但总体思路也比较清晰简单，如果理解了上面所说的跳跃表的原理，那么很容易理清楚插入节点时发生的几个动作 <em>（几乎跟链表类似）</em>：</p>
<ol>
<li>找到当前我需要插入的位置 <em>（其中包括相同 score 时的处理）</em>；</li>
<li>创建新节点，调整前后的指针指向，完成插入；</li>
</ol>
<p>为了方便阅读，我把源码 <code v-pre>t_zset.c/zslInsert</code> 定义的插入函数拆成了几个部分</p>
<h6 id="第一部分-声明需要存储的变量" tabindex="-1"><a class="header-anchor" href="#第一部分-声明需要存储的变量" aria-hidden="true">#</a> 第一部分：声明需要存储的变量</h6>
<div class="language-c line-numbers-mode" data-ext="c"><pre v-pre class="language-c"><code><span class="token comment">// 存储搜索路径</span>
...
<span class="token comment">// 存储经过的节点跨度</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h6 id="第二部分-搜索当前节点插入位置" tabindex="-1"><a class="header-anchor" href="#第二部分-搜索当前节点插入位置" aria-hidden="true">#</a> 第二部分：搜索当前节点插入位置</h6>
<div class="language-c line-numbers-mode" data-ext="c"><pre v-pre class="language-c"><code><span class="token function">serverAssert</span><span class="token punctuation">(</span><span class="token operator">!</span><span class="token function">isnan</span><span class="token punctuation">(</span>score<span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
x <span class="token operator">=</span> zsl<span class="token operator">-></span>header<span class="token punctuation">;</span>
<span class="token comment">// 逐步降级寻找目标节点，得到 "搜索路径"</span>
<span class="token keyword">for</span> <span class="token punctuation">(</span>i <span class="token operator">=</span> zsl<span class="token operator">-></span>level<span class="token operator">-</span><span class="token number">1</span><span class="token punctuation">;</span> i <span class="token operator">>=</span> <span class="token number">0</span><span class="token punctuation">;</span> i<span class="token operator">--</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token comment">// 如果 score 相等，还需要比较 value 值</span>
    ...
    <span class="token punctuation">}</span>
    <span class="token comment">// 记录 "搜索路径"</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>讨论：</strong> 有一种极端的情况，就是跳跃表中的所有 score 值都是一样，zset 的查找性能会不会退化为 O(n) 呢？</p>
<p>从上面的源码中我们可以发现 zset 的排序元素不只是看 score 值，也会比较 value 值 <em>（字符串比较）</em></p>
<h6 id="第三部分-生成插入节点" tabindex="-1"><a class="header-anchor" href="#第三部分-生成插入节点" aria-hidden="true">#</a> 第三部分：生成插入节点</h6>
<div class="language-c line-numbers-mode" data-ext="c"><pre v-pre class="language-c"><code><span class="token comment">/* we assume the element is not already inside, since we allow duplicated
...
<span class="token comment">// 如果随机生成的 level 超过了当前最大 level 需要更新跳跃表的信息</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h6 id="第四部分-重排前向指针" tabindex="-1"><a class="header-anchor" href="#第四部分-重排前向指针" aria-hidden="true">#</a> 第四部分：重排前向指针</h6>
<div class="language-c line-numbers-mode" data-ext="c"><pre v-pre class="language-c"><code><span class="token keyword">for</span> <span class="token punctuation">(</span>i <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span> i <span class="token operator">&lt;</span> level<span class="token punctuation">;</span> i<span class="token operator">++</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h5 id="节点删除实现" tabindex="-1"><a class="header-anchor" href="#节点删除实现" aria-hidden="true">#</a> 节点删除实现</h5>
<p>删除过程由源码中的 <code v-pre>t_zset.c/zslDeleteNode</code> 定义，和插入过程类似，都需要先把这个 <strong>&quot;搜索路径&quot;</strong> 找出来，然后对于每个层的相关节点重排一下前向后向指针，同时还要注意更新一下最高层数 <code v-pre>maxLevel</code>，直接放源码 <em>(如果理解了插入这里还是很容易理解的)</em>：</p>
<div class="language-c line-numbers-mode" data-ext="c"><pre v-pre class="language-c"><code><span class="token comment">/* Internal function used by zslDelete, zslDeleteByScore and zslDeleteByRank */</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h5 id="节点更新实现" tabindex="-1"><a class="header-anchor" href="#节点更新实现" aria-hidden="true">#</a> 节点更新实现</h5>
<p>当我们调用 <code v-pre>ZADD</code> 方法时，如果对应的 value 不存在，那就是插入过程，如果这个 value 已经存在，只是调整一下 score 的值，那就需要走一个更新流程。</p>
<p>假设这个新的 score 值并不会带来排序上的变化，那么就不需要调整位置，直接修改元素的 score 值就可以了，但是如果排序位置改变了，那就需要调整位置，该如何调整呢？</p>
<p>从源码 <code v-pre>t_zset.c/zsetAdd</code> 函数 <code v-pre>1350</code> 行左右可以看到，Redis 采用了一个非常简单的策略：</p>
<div class="language-c line-numbers-mode" data-ext="c"><pre v-pre class="language-c"><code><span class="token comment">/* Remove and re-insert when score changed. */</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>把这个元素删除再插入这个</strong>，需要经过两次路径搜索，从这一点上来看，Redis 的 <code v-pre>ZADD</code> 代码似乎还有进一步优化的空间。</p>
<h5 id="元素排名的实现" tabindex="-1"><a class="header-anchor" href="#元素排名的实现" aria-hidden="true">#</a> 元素排名的实现</h5>
<p>跳跃表本身是有序的，Redis 在 skiplist 的 forward 指针上进行了优化，给每一个 forward 指针都增加了 <code v-pre>span</code> 属性，用来 <strong>表示从前一个节点沿着当前层的 forward 指针跳到当前这个节点中间会跳过多少个节点</strong>。在上面的源码中我们也可以看到 Redis 在插入、删除操作时都会小心翼翼地更新 <code v-pre>span</code> 值的大小。</p>
<p>所以，沿着 <strong>&quot;搜索路径&quot;</strong>，把所有经过节点的跨度 <code v-pre>span</code> 值进行累加就可以算出当前元素的最终 rank 值了：</p>
<div class="language-c line-numbers-mode" data-ext="c"><pre v-pre class="language-c"><code><span class="token comment">/* Find the rank for an element by both score and key.
...
<span class="token keyword">unsigned</span> <span class="token keyword">long</span> <span class="token function">zslGetRank</span><span class="token punctuation">(</span>zskiplist <span class="token operator">*</span>zsl<span class="token punctuation">,</span> <span class="token keyword">double</span> score<span class="token punctuation">,</span> sds ele<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token keyword">for</span> <span class="token punctuation">(</span>i <span class="token operator">=</span> zsl<span class="token operator">-></span>level<span class="token operator">-</span><span class="token number">1</span><span class="token punctuation">;</span> i <span class="token operator">>=</span> <span class="token number">0</span><span class="token punctuation">;</span> i<span class="token operator">--</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token keyword">while</span> <span class="token punctuation">(</span>x<span class="token operator">-></span>level<span class="token punctuation">[</span>i<span class="token punctuation">]</span><span class="token punctuation">.</span>forward <span class="token operator">&amp;&amp;</span>
            ...
            <span class="token comment">// span 累加</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="⭐怎么保证-redis-挂掉之后再重启数据可以进行恢复-持久化策略" tabindex="-1"><a class="header-anchor" href="#⭐怎么保证-redis-挂掉之后再重启数据可以进行恢复-持久化策略" aria-hidden="true">#</a> ⭐怎么保证 Redis 挂掉之后再重启数据可以进行恢复？（持久化策略）</h3>
<blockquote>
<p>概述：</p>
<p>RDB持久化是把当前进程数据生成快照保存到磁盘上的过程; 针对RDB不适合实时持久化的问题，Redis提供了AOF持久化方式来解决.</p>
<p>AOF是“写后”日志，Redis先执行命令，把数据写入内存，然后才记录日志。日志里记录的是Redis收到的每一条命令，这些命令是以文本形式保存。</p>
...
<h4 id="什么是-rdb-持久化" tabindex="-1"><a class="header-anchor" href="#什么是-rdb-持久化" aria-hidden="true">#</a> 什么是 RDB 持久化？</h4>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>save <span class="token number">900</span> <span class="token number">1</span>           <span class="token comment">#在900秒(15分钟)之后，如果至少有1个key发生变化，Redis就会自动触发BGSAVE命令创建快照。</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="什么是-aof-持久化" tabindex="-1"><a class="header-anchor" href="#什么是-aof-持久化" aria-hidden="true">#</a> 什么是 AOF 持久化？</h4>
...
<h4 id="aof-重写了解吗" tabindex="-1"><a class="header-anchor" href="#aof-重写了解吗" aria-hidden="true">#</a> AOF 重写了解吗？</h4>
...
<h4 id="为什么会有混合持久化" tabindex="-1"><a class="header-anchor" href="#为什么会有混合持久化" aria-hidden="true">#</a> 为什么会有混合持久化？</h4>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220715160726101.png" alt="image-20220715160726101" tabindex="0" loading="lazy"><figcaption>image-20220715160726101</figcaption></figure>
...
<h3 id="缓存穿透-缓存雪崩-缓存击穿" tabindex="-1"><a class="header-anchor" href="#缓存穿透-缓存雪崩-缓存击穿" aria-hidden="true">#</a> 缓存穿透 &amp; 缓存雪崩 &amp; 缓存击穿</h3>
<h4 id="穿透" tabindex="-1"><a class="header-anchor" href="#穿透" aria-hidden="true">#</a> 穿透</h4>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220714212317883.png" alt="image-20220714212317883" tabindex="0" loading="lazy"><figcaption>image-20220714212317883</figcaption></figure>
<p>缓存穿透的发生一般有这两种情况：</p>
<ul>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220616204045863.png" alt="image-20220616204045863" tabindex="0" loading="lazy"><figcaption>image-20220616204045863</figcaption></figure>
...
<p>更多关于布隆过滤器的内容可以看我的这篇原创：<a href="https://javaguide.cn/cs-basics/data-structure/bloom-filter/" target="_blank" rel="noopener noreferrer">《不了解布隆过滤器？一文给你整的明明白白！》open in new window<ExternalLinkIcon/></a> ，强烈推荐，个人感觉网上应该找不到总结的这么明明白白的文章了</p>
<h4 id="【拓展1】布隆过滤器" tabindex="-1"><a class="header-anchor" href="#【拓展1】布隆过滤器" aria-hidden="true">#</a> 【拓展1】布隆过滤器</h4>
<p>什么是布隆过滤器？</p>
...
<figure><img src="@source/Java/eightpart/personal_images/布隆过滤器-bit数组.png" alt="布隆过滤器示意图" tabindex="0" loading="lazy"><figcaption>布隆过滤器示意图</figcaption></figure>
...
<p>举个简单的例子：</p>
<figure><img src="@source/Java/eightpart/personal_images/布隆过滤器-hash运算.png" alt="布隆过滤器hash计算" tabindex="0" loading="lazy"><figcaption>布隆过滤器hash计算</figcaption></figure>
...
<h4 id="【拓展2】布谷鸟过滤器-cuckoo-filter" tabindex="-1"><a class="header-anchor" href="#【拓展2】布谷鸟过滤器-cuckoo-filter" aria-hidden="true">#</a> 【拓展2】布谷鸟过滤器（Cuckoo Filter）</h4>
<h5 id="布谷鸟哈希" tabindex="-1"><a class="header-anchor" href="#布谷鸟哈希" aria-hidden="true">#</a> 布谷鸟哈希</h5>
<p>布谷鸟过滤器用更低的空间开销解决了布隆过滤器不能删除元素的问题，做到了更好的效果，具体的</p>
<ul>
<li>支持动态的添加和删除元素</li>
<li>提供了比传统布隆过滤器更高的查找性能，即使在接近满的情况下（比如空间利用率达到 95% 的时候）</li>
<li>比起商过滤器它更容易实现</li>
<li>如果要求误判率低于3%，它比布隆过滤器有更低的空间开销</li>
</ul>
<p>本质上来说它为解决哈希冲突提供了另一种策略，利用较少计算换取了较大空间。它具有占用空间小、查询迅速等特性。名称源于采取了一种和布谷鸟一样的养娃方法</p>
<blockquote>
<p>布谷鸟交配后，雌性<strong>布谷鸟</strong>就准备产蛋了，但它却不会自己筑巢。它会来到像知更<strong>鸟</strong>、刺嘴莺等那些比它小的<strong>鸟</strong>类的巢中，移走原来的那窝蛋中的一个，用自己的蛋来取而代之。相对于它的体形来说，它的蛋是偏小的，而且蛋上的斑纹同它混入的其他<strong>鸟</strong>的蛋也非常相似，所以不易被分辨出来。如果不是这样，它的蛋肯定会被扔出去。</p>
</blockquote>
<p>最原始的布谷鸟哈希方法是使用两个哈希函数对一个 <code v-pre>key</code>进行哈希，得到桶中的两个位置，此时</p>
<ul>
<li>如果两个位置都为为空则将 <code v-pre>key</code>随机存入其中一个位置</li>
<li>如果只有一个位置为空则存入为空的位置</li>
<li>如果都不为空，则随机踢出一个元素，踢出的元素再重新计算哈希找到相应的位置</li>
</ul>
<p>当然假如存在绝对的空间不足，那老是踢出也不是办法，所以一般会设置一个<strong>踢出阈值</strong>，如果在某次插入行为过程中连续踢出超过阈值，则进行扩容。</p>
<figure><img src="@source/Java/eightpart/personal_images/1617168-20210727195627534-1821849951.png" alt="image-20210727104910960" tabindex="0" loading="lazy"><figcaption>image-20210727104910960</figcaption></figure>
<h5 id="布谷鸟过滤器" tabindex="-1"><a class="header-anchor" href="#布谷鸟过滤器" aria-hidden="true">#</a> 布谷鸟过滤器</h5>
<figure><img src="@source/Java/eightpart/personal_images/1617168-20210727195625561-1153589445.jpg" alt="1" tabindex="0" loading="lazy"><figcaption>1</figcaption></figure>
<p>上图（a）(b)展示了一个基本的布谷鸟哈希表的插入操作，是由一个桶数组组成，每个插入项都有由散列函数h1(x)和h2(x)确定的两个候选桶，具体操作上文中已经描述，此处不再赘述。</p>
<p>而基本的布谷鸟过滤器也是由两个或者多个哈希函数构成，布谷鸟过滤器的布谷鸟哈希表的基本单位称为<strong>条目（entry）</strong>。 每个条目存储一个<strong>指纹（fingerprint）</strong>，指纹指的是使用一个哈希函数生成的n位比特位，n的具体大小由所能接受的误判率来设置，论文中的例子使用的是8bits的指纹大小。</p>
<p>哈希表由一个桶数组组成，其中一个桶可以有多个条目（比如上述图c中有四个条目）。而每个桶中有四个指纹位置，意味着一次哈希计算后布谷鸟有四个“巢“可用，而且四个巢是连续位置，可以更好的利用cpu高速缓存。也就是说每个桶的大小是4*8bits</p>
<h4 id="雪崩" tabindex="-1"><a class="header-anchor" href="#雪崩" aria-hidden="true">#</a> 雪崩</h4>
<p>什么是缓存雪崩？</p>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220616203658271.png" alt="image-20220616203658271" tabindex="0" loading="lazy"><figcaption>image-20220616203658271</figcaption></figure>
<p>通常，我们会使用缓存用于缓冲对 DB 的冲击，如果缓存宕机，所有请求将直接打在 DB，造成 DB 宕机——从而导致整个系统宕机。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220616203727609.png" alt="image-20220616203727609" tabindex="0" loading="lazy"><figcaption>image-20220616203727609</figcaption></figure>
<p><strong>2 种策略：</strong></p>
<ul>
...
<h4 id="击穿" tabindex="-1"><a class="header-anchor" href="#击穿" aria-hidden="true">#</a> 击穿</h4>
<p><strong>缓存击穿</strong>是指某一个热点数据缓存中没有但数据库中有数据（一般是缓存时间到期，比如秒杀活动，这类被频地访问的数据被称为热点数据）。这时由于并发用户特别多，同时读缓存没读到数据（<strong>某个热点数据过期</strong>了，此时大量的请求访问了该热点数据，就无法从缓存中读取），就去数据库去取数据，引起数据库压力瞬间增大，被高并发的请求冲垮，这就是<strong>缓存击穿</strong>的问题。</p>
<p>解决：</p>
<ol>
<li><strong>设置热点数据永远不过期</strong>。</li>
<li><strong>接口限流与熔断</strong>，降级。重要的接口一定要做好限流策略，防止用户恶意刷接口，同时要降级准备，当接口中的某些服务不可用时候，进行熔断，失败快速返回机制。</li>
<li><strong>设置互斥锁</strong>。在并发的多个请求中，只有第一个请求线程能拿到锁并执行数据库查询操作，其他的线程拿不到锁就阻塞等着，等到第一个线程将数据写入缓存后，直接走缓存。（可以使用 Redis 分布式锁）</li>
</ol>
<figure><img src="@source/Java/eightpart/personal_images/image-20220714212532910.png" alt="image-20220714212532910" tabindex="0" loading="lazy"><figcaption>image-20220714212532910</figcaption></figure>
<h3 id="主从复制方案是怎么做的" tabindex="-1"><a class="header-anchor" href="#主从复制方案是怎么做的" aria-hidden="true">#</a> 主从复制方案是怎么做的？</h3>
<figure><img src="@source/Java/eightpart/personal_images/image-20220714220149168.png" alt="image-20220714220149168" tabindex="0" loading="lazy"><figcaption>image-20220714220149168</figcaption></figure>
...
<p>主服务器可以进行读写操作，当发生写操作时自动将写操作同步给从服务器，而从服务器一般是只读，并接受主服务器同步过来写操作命令，然后执行这条命令。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220714220158025.png" alt="image-20220714220158025" tabindex="0" loading="lazy"><figcaption>image-20220714220158025</figcaption></figure>
<p>也就是说，所有的数据修改只在主服务器上进行，然后将最新的数据同步给从服务器，这样就使得主从服务器的数据是一致的。</p>
<p>同步这两个字说的简单，但是这个同步过程并没有想象中那么简单，要考虑的事情不是一两个。</p>
<p>我们先来看看，主从服务器间的第一次同步是如何工作的？</p>
<h4 id="第一次同步" tabindex="-1"><a class="header-anchor" href="#第一次同步" aria-hidden="true">#</a> 第一次同步</h4>
<p>多台服务器之间要通过什么方式来确定谁是主服务器，或者谁是从服务器呢？</p>
<p>我们可以使用 <code v-pre>replicaof</code>（Redis 5.0 之前使用 slaveof）命令形成主服务器和从服务器的关系。</p>
<p>比如，现在有服务器 A 和 服务器 B，我们在服务器 B 上执行下面这条命令：</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code># 服务器 B 执行这条命令
replicaof &lt;服务器 A 的 IP 地址> &lt;服务器 A 的 Redis 端口号>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>接着，服务器 B 就会变成服务器 A 的「从服务器」，然后与主服务器进行第一次同步。</p>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220714220220342.png" alt="image-20220714220220342" tabindex="0" loading="lazy"><figcaption>image-20220714220220342</figcaption></figure>
<blockquote>
<p>接下来，我在具体介绍每一个阶段都做了什么。</p>
</blockquote>
<p><em>第一阶段：建立链接、协商同步</em></p>
<p>执行了 replicaof 命令后，从服务器就会给主服务器发送 <code v-pre>psync</code> 命令，表示要进行数据同步。</p>
...
<p>主服务器收到 psync 命令后，会用 <code v-pre>FULLRESYNC</code> 作为响应命令返回给对方。</p>
...
<p>接着，主服务器会执行 <code v-pre>bgsave</code> 命令来生成 RDB 文件，然后把文件发送给从服务器。</p>
...
<p>在主服务器生成的 RDB 文件发送完，从服务器加载完 RDB 文件后，然后将 replication buffer 缓冲区里所记录的写操作命令发送给从服务器，然后「从服务器」重新执行这些操作，至此主从服务器的数据就一致了。</p>
<p>至此，主从服务器的第一次同步的工作就完成了。</p>
<h4 id="命令传播" tabindex="-1"><a class="header-anchor" href="#命令传播" aria-hidden="true">#</a> 命令传播</h4>
<p>主从服务器在完成第一次同步后，双方之间就会维护一个 TCP 连接。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220714220259046.png" alt="image-20220714220259046" tabindex="0" loading="lazy"><figcaption>image-20220714220259046</figcaption></figure>
...
<h4 id="分摊主服务器的压力" tabindex="-1"><a class="header-anchor" href="#分摊主服务器的压力" aria-hidden="true">#</a> 分摊主服务器的压力</h4>
<p>在前面的分析中，我们可以知道主从服务器在第一次数据同步的过程中，主服务器会做两件耗时的操作：生成 RDB 文件和传输 RDB 文件。</p>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220714220315731.png" alt="image-20220714220315731" tabindex="0" loading="lazy"><figcaption>image-20220714220315731</figcaption></figure>
<p>通过这种方式，<strong>主服务器生成 RDB 和传输 RDB 的压力可以分摊到充当经理角色的从服务器</strong>。</p>
<p>那具体怎么做到的呢？</p>
<p>其实很简单，我们在「从服务器」上执行下面这条命令，使其作为目标服务器的从服务器：</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>replicaof &lt;目标服务器的IP> 6379
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>此时如果目标服务器本身也是「从服务器」，那么该目标服务器就会成为「经理」的角色，不仅可以接受主服务器同步的数据，也会把数据同步给自己旗下的从服务器，从而减轻主服务器的负担。</p>
<h4 id="增量复制" tabindex="-1"><a class="header-anchor" href="#增量复制" aria-hidden="true">#</a> 增量复制</h4>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220714220333043.png" alt="image-20220714220333043" tabindex="0" loading="lazy"><figcaption>image-20220714220333043</figcaption></figure>
...
<p>网络恢复后的增量复制过程如下图：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220714220340040.png" alt="image-20220714220340040" tabindex="0" loading="lazy"><figcaption>image-20220714220340040</figcaption></figure>
<p>主要有三个步骤：</p>
<ul>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220714220348740.png" alt="image-20220714220348740" tabindex="0" loading="lazy"><figcaption>image-20220714220348740</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220714220356331.png" alt="image-20220714220356331" tabindex="0" loading="lazy"><figcaption>image-20220714220356331</figcaption></figure>
<p>我来解释下这个公式的意思：</p>
<ul>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>repl-backlog-size 1mb
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h4 id="总结" tabindex="-1"><a class="header-anchor" href="#总结" aria-hidden="true">#</a> 总结</h4>
...
<h3 id="sentinel-哨兵-有什么作用" tabindex="-1"><a class="header-anchor" href="#sentinel-哨兵-有什么作用" aria-hidden="true">#</a> Sentinel（哨兵）  有什么作用？</h3>
<blockquote>
<p>Redis Sentinel，即Redis哨兵，在Redis 2.8版本开始引入。哨兵的核心功能是主节点的自动故障转移。</p>
</blockquote>
<p>下图是一个典型的哨兵集群监控的逻辑图：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220617154744863.png" alt="image-20220617154744863" tabindex="0" loading="lazy"><figcaption>image-20220617154744863</figcaption></figure>
...
<h3 id="为什么要哨兵" tabindex="-1"><a class="header-anchor" href="#为什么要哨兵" aria-hidden="true">#</a> 为什么要哨兵？</h3>
<p>在 Redis 的主从架构中，由于主从模式是读写分离的，如果主节点（master）挂了，那么将没有主节点来服务客户端的写操作请求，也没有主节点给从节点（slave）进行数据同步了。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220714220520655.png" alt="image-20220714220520655" tabindex="0" loading="lazy"><figcaption>image-20220714220520655</figcaption></figure>
...
<h4 id="哨兵机制是如何工作的" tabindex="-1"><a class="header-anchor" href="#哨兵机制是如何工作的" aria-hidden="true">#</a> 哨兵机制是如何工作的？</h4>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220714220539777.png" alt="image-20220714220539777" tabindex="0" loading="lazy"><figcaption>image-20220714220539777</figcaption></figure>
<p>所以，我们重点要学习这三件事情：</p>
<ul>
...
<h4 id="如何判断主节点真的故障了" tabindex="-1"><a class="header-anchor" href="#如何判断主节点真的故障了" aria-hidden="true">#</a> 如何判断主节点真的故障了？</h4>
<p>哨兵会每隔 1 秒给所有主从节点发送 PING 命令，当主从节点收到 PING 命令后，会发送一个响应命令给哨兵，这样就可以判断它们是否在正常运行。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220714220557761.png" alt="image-20220714220557761" tabindex="0" loading="lazy"><figcaption>image-20220714220557761</figcaption></figure>
<p>如果主节点或者从节点没有在规定的时间内响应哨兵的 PING 命令，哨兵就会将它们标记为「<strong>主观下线</strong>」。这个「规定的时间」是配置项  <code v-pre>down-after-milliseconds</code> 参数设定的，单位是毫秒。</p>
<blockquote>
<p>主观下线？难道还有客观下线？</p>
</blockquote>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220714220606567.png" alt="image-20220714220606567" tabindex="0" loading="lazy"><figcaption>image-20220714220606567</figcaption></figure>
...
<h4 id="由哪个哨兵进行主从故障转移" tabindex="-1"><a class="header-anchor" href="#由哪个哨兵进行主从故障转移" aria-hidden="true">#</a> 由哪个哨兵进行主从故障转移？</h4>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220714220628098.png" alt="image-20220714220628098" tabindex="0" loading="lazy"><figcaption>image-20220714220628098</figcaption></figure>
...
<h4 id="主从故障转移的过程是怎样的" tabindex="-1"><a class="header-anchor" href="#主从故障转移的过程是怎样的" aria-hidden="true">#</a> 主从故障转移的过程是怎样的？</h4>
<p>在哨兵集群中通过投票的方式，选举出了哨兵 leader 后，就可以进行主从故障转移的过程了，如下图：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220714222351206.png" alt="image-20220714222351206" tabindex="0" loading="lazy"><figcaption>image-20220714222351206</figcaption></figure>
...
<blockquote>
<p>这块具体还是看<a href="https://xiaolincoding.com/redis/cluster/sentinel.html#%E6%AD%A5%E9%AA%A4%E4%B8%80-%E9%80%89%E5%87%BA%E6%96%B0%E4%B8%BB%E8%8A%82%E7%82%B9" target="_blank" rel="noopener noreferrer">https://xiaolincoding.com/redis/cluster/sentinel.html#步骤一-选出新主节点<ExternalLinkIcon/></a></p>
</blockquote>
<h4 id="哨兵集群是如何组成的" tabindex="-1"><a class="header-anchor" href="#哨兵集群是如何组成的" aria-hidden="true">#</a> 哨兵集群是如何组成的？</h4>
...
<div class="language-c line-numbers-mode" data-ext="c"><pre v-pre class="language-c"><code>sentinel monitor <span class="token operator">&lt;</span>master<span class="token operator">-</span>name<span class="token operator">></span> <span class="token operator">&lt;</span>ip<span class="token operator">></span> <span class="token operator">&lt;</span>redis<span class="token operator">-</span>port<span class="token operator">></span> <span class="token operator">&lt;</span>quorum<span class="token operator">></span> 
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>不需要填其他哨兵节点的信息，我就好奇它们是如何感知对方的，又是如何组成哨兵集群的？</p>
<p>后面才了解到，<strong>哨兵节点之间是通过 Redis 的发布者/订阅者机制来相互发现的</strong>。</p>
<p>在主从集群中，主节点上有一个名为 <code v-pre>__sentinel__:hello</code>的频道，不同哨兵就是通过它来相互发现，实现互相通信的。</p>
<p>在下图中，哨兵 A 把自己的 IP 地址和端口的信息发布到 <code v-pre>__sentinel__:hello</code> 频道上，哨兵 B 和 C 订阅了该频道。那么此时，哨兵 B 和 C 就可以从这个频道直接获取哨兵 A 的 IP 地址和端口号。然后，哨兵 B、C 可以和哨兵 A 建立网络连接。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220714222643304.png" alt="image-20220714222643304" tabindex="0" loading="lazy"><figcaption>image-20220714222643304</figcaption></figure>
...
<p>如下图所示，哨兵 B 给主节点发送 INFO  命令，主节点接受到这个命令后，就会把从节点列表返回给哨兵。接着，哨兵就可以根据从节点列表中的连接信息，和每个从节点建立连接，并在这个连接上持续地对从节点进行监控。哨兵 A 和 C 可以通过相同的方法和从节点建立连接。<img src="@source/Java/eightpart/personal_images/image-20220714222836699.png" alt="image-20220714222836699" loading="lazy"></p>
...
<h3 id="cluster-集群-的原理你能讲一下吗" tabindex="-1"><a class="header-anchor" href="#cluster-集群-的原理你能讲一下吗" aria-hidden="true">#</a> Cluster（集群）的原理你能讲一下吗？</h3>
<p>哨兵模式解决了主从复制不能自动故障转移、达不到高可用的问题，但还是存在主节点的写能力、容量受限于单机配置的问题。而cluster模式实现了Redis的分布式存储，每个节点存储不同的内容，解决主节点的写能力、容量受限于单机配置的问题。</p>
...
<figure><img src="@source/Java/eightpart/personal_images/cluster_slots.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
...
<p>16379 端口号是用来进行节点间通信的，也就是 cluster bus 的东西，cluster bus 的通信，用来进行故障检测、配置更新、故障转移授权。cluster bus 用了另外一种二进制的协议，<code v-pre>gossip</code> 协议，用于节点间进行高效的数据交换，占用更少的网络带宽和处理时间。</p>
<p><strong>优点：</strong></p>
<ul>
<li>无中心架构，<strong>支持动态扩</strong>容；</li>
<li>数据按照 <code v-pre>slot</code>存储分布在多个节点，节点间数据共享，<strong>可动态调整数据分布</strong>；</li>
<li><strong>高可用性</strong>。部分节点不可用时，集群仍可用。集群模式能够实现自动故障转移（failover），节点之间通过 <code v-pre>gossip</code>协议交换状态信息，用投票机制完成 <code v-pre>Slave</code>到 <code v-pre>Master</code>的角色转换。</li>
</ul>
<p><strong>缺点：</strong></p>
<ul>
<li><strong>不支持批量操作</strong>（pipeline）。</li>
<li>数据通过异步复制，<strong>不保证数据的强一致性</strong>。</li>
<li><strong>事务操作支持有限</strong>，只支持多 <code v-pre>key</code>在同一节点上的事务操作，当多个 <code v-pre>key</code>分布于不同的节点上时无法使用事务功能。</li>
<li><code v-pre>key</code>作为数据分区的最小粒度，不能将一个很大的键值对象如 <code v-pre>hash</code>、<code v-pre>list</code>等映射到不同的节点。</li>
...
<p>16379 端口号是用来进行节点间通信的，也就是 cluster bus 的东西，cluster bus 的通信，用来进行故障检测、配置更新、故障转移授权。cluster bus 用了另外一种二进制的协议， <code v-pre>gossip</code> 协议，用于节点间进行高效的数据交换，占用更少的网络带宽和处理时间。</p>
<h4 id="节点间的内部通信机制" tabindex="-1"><a class="header-anchor" href="#节点间的内部通信机制" aria-hidden="true">#</a> 节点间的内部通信机制</h4>
<h5 id="基本通信原理" tabindex="-1"><a class="header-anchor" href="#基本通信原理" aria-hidden="true">#</a> 基本通信原理</h5>
<p>集群元数据的维护有两种方式：集中式、Gossip 协议。Redis cluster 节点间采用 gossip 协议进行通信。</p>
<p><strong>集中式</strong>是将集群元数据（节点信息、故障等等）集中存储在某个节点上。集中式元数据集中存储的一个典型代表，就是大数据领域的 <code v-pre>storm</code> 。它是分布式的大数据实时计算引擎，是集中式的元数据存储的结构，底层基于 zookeeper（分布式协调的中间件）对所有元数据进行存储维护。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220715143320253.png" alt="image-20220715143320253" tabindex="0" loading="lazy"><figcaption>image-20220715143320253</figcaption></figure>
<p>Redis 维护集群元数据采用另一个方式， <code v-pre>gossip</code> 协议，所有节点都持有一份元数据，不同的节点如果出现了元数据的变更，就不断将元数据发送给其它的节点，让其它节点也进行元数据的变更。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220715143327361.png" alt="image-20220715143327361" tabindex="0" loading="lazy"><figcaption>image-20220715143327361</figcaption></figure>
...
<ul>
<li>10000 端口：每个节点都有一个专门用于节点间通信的端口，就是自己提供服务的端口号+10000，比如 7001，那么用于节点间通信的就是 17001 端口。每个节点每隔一段时间都会往另外几个节点发送 <code v-pre>ping</code> 消息，同时其它几个节点接收到 <code v-pre>ping</code> 之后返回 <code v-pre>pong</code> 。</li>
<li>交换的信息：信息包括故障信息，节点的增加和删除，hash slot 信息等等。</li>
</ul>
<h5 id="gossip-协议" tabindex="-1"><a class="header-anchor" href="#gossip-协议" aria-hidden="true">#</a> gossip 协议</h5>
<p>gossip 协议包含多种消息，包含 <code v-pre>ping</code> , <code v-pre>pong</code> , <code v-pre>meet</code> , <code v-pre>fail</code> 等等。</p>
<ul>
<li>meet：某个节点发送 meet 给新加入的节点，让新节点加入集群中，然后新节点就会开始与其它节点进行通信。</li>
</ul>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>Redis-trib.rb add-node
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>其实内部就是发送了一个 gossip meet 消息给新加入的节点，通知那个节点去加入我们的集群。</p>
<ul>
...
<h5 id="ping-消息深入" tabindex="-1"><a class="header-anchor" href="#ping-消息深入" aria-hidden="true">#</a> ping 消息深入</h5>
<p>ping 时要携带一些元数据，如果很频繁，可能会加重网络负担。</p>
<p>每个节点每秒会执行 10 次 ping，每次会选择 5 个最久没有通信的其它节点。当然如果发现某个节点通信延时达到了 <code v-pre>cluster_node_timeout / 2</code> ，那么立即发送 ping，避免数据交换延时过长，落后的时间太长了。比如说，两个节点之间都 10 分钟没有交换数据了，那么整个集群处于严重的元数据不一致的情况，就会有问题。所以 <code v-pre>cluster_node_timeout</code> 可以调节，如果调得比较大，那么会降低 ping 的频率。</p>
<p>每次 ping，会带上自己节点的信息，还有就是带上 1/10 其它节点的信息，发送出去，进行交换。至少包含 <code v-pre>3</code> 个其它节点的信息，最多包含 <code v-pre>总节点数减 2</code> 个其它节点的信息。</p>
<h4 id="分布式寻址算法" tabindex="-1"><a class="header-anchor" href="#分布式寻址算法" aria-hidden="true">#</a> 分布式寻址算法</h4>
<ul>
<li>hash 算法（大量缓存重建）</li>
<li>一致性 hash 算法（自动缓存迁移）+ 虚拟节点（自动负载均衡）</li>
<li>Redis cluster 的 hash slot 算法</li>
</ul>
<h5 id="hash-算法" tabindex="-1"><a class="header-anchor" href="#hash-算法" aria-hidden="true">#</a> hash 算法</h5>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220715143507698.png" alt="image-20220715143507698" tabindex="0" loading="lazy"><figcaption>image-20220715143507698</figcaption></figure>
<h5 id="一致性-hash-算法" tabindex="-1"><a class="header-anchor" href="#一致性-hash-算法" aria-hidden="true">#</a> 一致性 hash 算法</h5>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220715143520445.png" alt="image-20220715143520445" tabindex="0" loading="lazy"><figcaption>image-20220715143520445</figcaption></figure>
<h5 id="redis-cluster-的-hash-slot-算法" tabindex="-1"><a class="header-anchor" href="#redis-cluster-的-hash-slot-算法" aria-hidden="true">#</a> Redis cluster 的 hash slot 算法</h5>
<p>Redis cluster 有固定的 <code v-pre>16384</code> 个 hash slot，对每个 <code v-pre>key</code> 计算 <code v-pre>CRC16</code> 值，然后对 <code v-pre>16384</code> 取模，可以获取 key 对应的 hash slot。</p>
<p>Redis cluster 中每个 master 都会持有部分 slot，比如有 3 个 master，那么可能每个 master 持有 5000 多个  hash slot。hash slot 让 node 的增加和移除很简单，增加一个 master，就将其他 master 的 hash slot 移动部分过去，减少一个 master，就将它的 hash slot 移动到其他 master 上去。移动 hash slot  的成本是非常低的。客户端的 api，可以对指定的数据，让他们走同一个 hash slot，通过 <code v-pre>hash tag</code> 来实现。</p>
<p>任何一台机器宕机，另外两个节点，不影响的。因为 key 找的是 hash slot，不是机器。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220715143532988.png" alt="image-20220715143532988" tabindex="0" loading="lazy"><figcaption>image-20220715143532988</figcaption></figure>
<h4 id="redis-cluster-的高可用与主备切换原理" tabindex="-1"><a class="header-anchor" href="#redis-cluster-的高可用与主备切换原理" aria-hidden="true">#</a> Redis cluster 的高可用与主备切换原理</h4>
<p>Redis cluster 的高可用的原理，几乎跟哨兵是类似的。</p>
<h5 id="判断节点宕机" tabindex="-1"><a class="header-anchor" href="#判断节点宕机" aria-hidden="true">#</a> 判断节点宕机</h5>
<p>如果一个节点认为另外一个节点宕机，那么就是 <code v-pre>pfail</code> ，<strong>主观宕机</strong>。如果多个节点都认为另外一个节点宕机了，那么就是 <code v-pre>fail</code> ，<strong>客观宕机</strong>，跟哨兵的原理几乎一样，sdown，odown。</p>
<p>在 <code v-pre>cluster-node-timeout</code> 内，某个节点一直没有返回 <code v-pre>pong</code> ，那么就被认为 <code v-pre>pfail</code> 。</p>
<p>如果一个节点认为某个节点 <code v-pre>pfail</code> 了，那么会在 <code v-pre>gossip ping</code> 消息中， <code v-pre>ping</code> 给其他节点，如果<strong>超过半数</strong>的节点都认为 <code v-pre>pfail</code> 了，那么就会变成 <code v-pre>fail</code> 。</p>
<h5 id="从节点过滤" tabindex="-1"><a class="header-anchor" href="#从节点过滤" aria-hidden="true">#</a> 从节点过滤</h5>
<p>对宕机的 master node，从其所有的 slave node 中，选择一个切换成 master node。</p>
<p>检查每个 slave node 与 master node 断开连接的时间，如果超过了 <code v-pre>cluster-node-timeout * cluster-slave-validity-factor</code> ，那么就<strong>没有资格</strong>切换成 <code v-pre>master</code> 。</p>
<h5 id="从节点选举" tabindex="-1"><a class="header-anchor" href="#从节点选举" aria-hidden="true">#</a> 从节点选举</h5>
<p>每个从节点，都根据自己对 master 复制数据的 offset，来设置一个选举时间，offset 越大（复制数据越多）的从节点，选举时间越靠前，优先进行选举。</p>
<p>所有的 master node 开始 slave 选举投票，给要进行选举的 slave 进行投票，如果大部分 master node <code v-pre>（N/2 + 1）</code> 都投票给了某个从节点，那么选举通过，那个从节点可以切换成 master。</p>
<p>从节点执行主备切换，从节点切换为主节点。</p>
<h5 id="与哨兵比较" tabindex="-1"><a class="header-anchor" href="#与哨兵比较" aria-hidden="true">#</a> 与哨兵比较</h5>
<p>整个流程跟哨兵相比，非常类似，所以说，Redis cluster 功能强大，直接集成了 replication 和 sentinel 的功能。</p>
<h3 id="⭐mysql-和-redis-怎么保持数据一致" tabindex="-1"><a class="header-anchor" href="#⭐mysql-和-redis-怎么保持数据一致" aria-hidden="true">#</a> ⭐MySQL 和 Redis 怎么保持数据一致?</h3>
<p>对于缓存和数据库的操作，主要有以下两种方式。</p>
<h4 id="先删缓存-再更新数据库" tabindex="-1"><a class="header-anchor" href="#先删缓存-再更新数据库" aria-hidden="true">#</a> 先删缓存，再更新数据库</h4>
<p>举个 🌰 假设某个用户的年龄是 20，请求 A 要更新用户年龄为 21，所以它会删除缓存中的内容。这时，另一个请求 B  要读取这个用户的年龄，它查询缓存发现未命中后，会从数据库中读取到年龄为 20，并且写入到缓存中，然后请求 A 继续更改数据库，将用户的年龄更新为 21。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220715152802050.png" alt="image-20220715152802050" tabindex="0" loading="lazy"><figcaption>image-20220715152802050</figcaption></figure>
<p>最终，该用户年龄在缓存中是 20（旧值），在数据库中是 21（新值），缓存和数据库的数据不一致。</p>
<p>可以看到，<strong>先删除缓存，再更新数据库，在「读 + 写」并发的时候，还是会出现缓存和数据库的数据不一致的问题</strong>。</p>
<h5 id="解决方案-延时双删" tabindex="-1"><a class="header-anchor" href="#解决方案-延时双删" aria-hidden="true">#</a> 解决方案:延时双删</h5>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220715152925965.png" alt="image-20220715152925965" tabindex="0" loading="lazy"><figcaption>image-20220715152925965</figcaption></figure>
...
<p>假如某个用户数据在缓存中不存在，请求 A 读取数据时从数据库中查询到年龄为 20，在未写入缓存中时另一个请求 B 更新数据。它更新数据库中的年龄为 21，并且清空缓存。这时请求 A 把从数据库中读到的年龄为 20 的数据写入到缓存中。<img src="@source/Java/eightpart/personal_images/image-20220715152956657.png" alt="image-20220715152956657" loading="lazy"></p>
...
<h5 id="解决方案1-消息队列" tabindex="-1"><a class="header-anchor" href="#解决方案1-消息队列" aria-hidden="true">#</a> 解决方案1:消息队列</h5>
<p>这是网上很多文章里都有写过的方案。但是这个方案的缺陷会更明显一点。</p>
<p>先更新数据库，成功后往消息队列发消息，消费到消息后再删除缓存，借助消息队列的重试机制来实现，达到最终一致性的效果。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220715153053443.png" alt="image-20220715153053443" tabindex="0" loading="lazy"><figcaption>image-20220715153053443</figcaption></figure>
<p>这个解决方案其实问题更多。</p>
<ol>
<li>引入消息中间件之后，问题更复杂了，怎么保证消息不丢失更麻烦</li>
<li>就算更新数据库和删除缓存都没有发生问题，消息的延迟也会带来短暂的不一致性，不过这个延迟相对来说还是可以接受的</li>
</ol>
<h5 id="解决方案2-进阶版消息队列" tabindex="-1"><a class="header-anchor" href="#解决方案2-进阶版消息队列" aria-hidden="true">#</a> 解决方案2:进阶版消息队列</h5>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220715153124553.png" alt="image-20220715153124553" tabindex="0" loading="lazy"><figcaption>image-20220715153124553</figcaption></figure>
<h5 id="解决方案3-设置缓存过期时间" tabindex="-1"><a class="header-anchor" href="#解决方案3-设置缓存过期时间" aria-hidden="true">#</a> 解决方案3:设置缓存过期时间</h5>
...
<p>详细问题可以看：<a href="https://xiaolincoding.com/redis/architecture/mysql_redis_consistency.html#%E5%85%88%E6%9B%B4%E6%96%B0%E6%95%B0%E6%8D%AE%E5%BA%93-%E8%BF%98%E6%98%AF%E5%85%88%E5%88%A0%E9%99%A4%E7%BC%93%E5%AD%98" target="_blank" rel="noopener noreferrer">数据库和缓存如何保证一致性？<ExternalLinkIcon/></a></p>
<h3 id="⭐️redis如何实现分布式锁" tabindex="-1"><a class="header-anchor" href="#⭐️redis如何实现分布式锁" aria-hidden="true">#</a> ⭐️Redis如何实现分布式锁？</h3>
<p><strong>分布式锁</strong>：当多个进程不在同一个系统中(比如分布式系统中控制共享资源访问)，用分布式锁控制多个进程对资源的访问。</p>
<p><a href="https://redis.io/docs/reference/patterns/distributed-locks/" target="_blank" rel="noopener noreferrer">Redis的官网<ExternalLinkIcon/></a>上对使用分布式锁提出至少需要满足如下三个要求：</p>
<ol>
...
<h4 id="最简化版本" tabindex="-1"><a class="header-anchor" href="#最简化版本" aria-hidden="true">#</a> 最简化版本</h4>
<p>首先，当然是搭建一个最简单的实现方式，直接用Redis的setnx命令，这个命令的语法是：<strong>setnx key value</strong></p>
<p>如果key不存在，则会将key设置为value，并返回1；如果key存在，不会有任务影响，返回0。</p>
<p>基于这个特性，我们就可以用setnx实现加锁的目的：通过setnx加锁，加锁之后其他服务无法加锁，用完之后，再通过delete解锁，深藏功与名</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220709164349575.png" alt="image-20220709164349575" tabindex="0" loading="lazy"><figcaption>image-20220709164349575</figcaption></figure>
<h4 id="支持过期时间" tabindex="-1"><a class="header-anchor" href="#支持过期时间" aria-hidden="true">#</a> 支持过期时间</h4>
<p>最简化版本有一个问题：<strong>如果获取锁的服务挂掉了，那么锁就一直得不到释放</strong>，就像石沉大海，杳无音信。所以，我们需要一个超时来兜底。</p>
<p>Redis中有expire命令，用来设置一个key的超时时间。但是setnx和expire不具备原子性，如果setnx获取锁之后，服务挂掉，依旧是泥牛入海。</p>
<p>很自然，我们会想到，set和expire，有没有原子操作？</p>
<p>当然有，Redis早就考虑到了这种场景，推出了如下执行语句：<strong>set key value nx ex seconds</strong></p>
<p>nx表示具备setnx特定，ex表示增加了过期时间，最后一个参数就是过期时间的值。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220709164418917.png" alt="image-20220709164418917" tabindex="0" loading="lazy"><figcaption>image-20220709164418917</figcaption></figure>
<p>能够支持过期时间，目前这个锁基本上是能用了。</p>
<p>但是存在一个问题：会存在服务A释放掉服务B的锁的可能。</p>
<h4 id="加上owner" tabindex="-1"><a class="header-anchor" href="#加上owner" aria-hidden="true">#</a> 加上owner</h4>
<p>我们来试想一下如下场景：服务A获取了锁，由于业务流程比较长，或者网络延迟、GC卡顿等原因，导致锁过期，而业务还会继续进行。这时候，业务B已经拿到了锁，准备去执行，这个时候服务A恢复过来并做完了业务，就会释放锁，而B却还在继续执行。</p>
<p>在真实的分布式场景中，可能存在几十个竞争者，那么上述情况发生概率就很高，导致同一份资源频繁被不同竞争者同时访问，分布式锁也就失去了意义。</p>
<p>基于这个场景，我们可以发现，问题关键在于，竞争者可以释放其他人的锁。那么在异常情况下，就会出现问题，所以我们可以进一步给出解决方案：<strong>分布式锁需要满足谁申请谁释放原则，不能释放别人的锁，也就是说，分布式锁，是要有归属的</strong></p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220709164452895.png" alt="image-20220709164452895" tabindex="0" loading="lazy"><figcaption>image-20220709164452895</figcaption></figure>
<h4 id="引入lua" tabindex="-1"><a class="header-anchor" href="#引入lua" aria-hidden="true">#</a> 引入Lua</h4>
<p>加入owner后的版本可以称得上是完善了吗？还有没有什么隐患呢？</p>
<p>我也不卖关子了，到这一步其实还存在一个小问题，我们完整的流程是竞争者获取锁执行任务，执行完毕后检查锁是不是自己的，最后进行释放。</p>
<p>流程一梳理，你们肯定明白了，执行完毕后，<strong>检查锁，再释放，这些操作不是原子化的。</strong></p>
<p><strong>可能锁获取时还是自己的，删除时却已经是别人的</strong>了。这可怎么办呢？</p>
<p>Redis可没有直接提供这种场景原子化的操作啊。遇事不要慌，仔细想一想，Redis是不是还有个特性，专门整合原子操作，对，就是它——<strong>Lua</strong></p>
<p>Redis➕Lua，可以说是专门为解决原子问题而生。</p>
<p>有了Lua的特性，Redis才真正在分布式锁、秒杀等场景，有了用武之地，下面便是改造之后的流程：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220709164528516.png" alt="image-20220709164528516" tabindex="0" loading="lazy"><figcaption>image-20220709164528516</figcaption></figure>
<p>其实到了这一步，分布式锁的前三个特性：对称性、安全性、可靠性，就满足了。可以说是一个可用的分布式锁了，能满足大多数场景的需要</p>
<h4 id="redisson" tabindex="-1"><a class="header-anchor" href="#redisson" aria-hidden="true">#</a> Redisson</h4>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220709164742280.png" alt="image-20220709164742280" tabindex="0" loading="lazy"><figcaption>image-20220709164742280</figcaption></figure>
<p>只要线程一加锁成功，就会启动一个 <code v-pre>watch dog</code>看门狗，它是一个后台线程，会每隔10秒检查一下，如果线程1还持有锁，那么就会不断的延长锁key的生存时间。因此，Redisson就是使用watch dog解决了**「锁过期释放，业务没执行完」**问题。</p>
<h4 id="redlock-redisson" tabindex="-1"><a class="header-anchor" href="#redlock-redisson" aria-hidden="true">#</a> Redlock+Redisson</h4>
<p>其实Redis一般都是集群部署的：</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220709164811068.png" alt="image-20220709164811068" tabindex="0" loading="lazy"><figcaption>image-20220709164811068</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220709164830756.png" alt="image-20220709164830756" tabindex="0" loading="lazy"><figcaption>image-20220709164830756</figcaption></figure>
<p>RedLock的实现步骤:如下</p>
<blockquote>
<p>❝</p>
<ul>
...
<h4 id="zookeeper分布式锁" tabindex="-1"><a class="header-anchor" href="#zookeeper分布式锁" aria-hidden="true">#</a> Zookeeper分布式锁</h4>
...
<h5 id="zk获取锁过程" tabindex="-1"><a class="header-anchor" href="#zk获取锁过程" aria-hidden="true">#</a> zk获取锁过程</h5>
<p>当第一个客户端请求过来时，Zookeeper客户端会创建一个持久节点<code v-pre>locks</code>。如果它（Client1）想获得锁，需要在<code v-pre>locks</code>节点下创建一个顺序节点<code v-pre>lock1</code>.如图</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220725154831233.png" alt="image-20220725154831233" tabindex="0" loading="lazy"><figcaption>image-20220725154831233</figcaption></figure>
<p>接着，客户端Client1会查找<code v-pre>locks</code>下面的所有临时顺序子节点，判断自己的节点<code v-pre>lock1</code>是不是排序最小的那一个，如果是，则成功获得锁。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220725154850768.png" alt="image-20220725154850768" tabindex="0" loading="lazy"><figcaption>image-20220725154850768</figcaption></figure>
<p>这时候如果又来一个客户端client2前来尝试获得锁，它会在locks下再创建一个临时节点<code v-pre>lock2</code></p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220725154905321.png" alt="image-20220725154905321" tabindex="0" loading="lazy"><figcaption>image-20220725154905321</figcaption></figure>
...
<figure><img src="@source/Java/eightpart/personal_images/image-20220725154920071.png" alt="image-20220725154920071" tabindex="0" loading="lazy"><figcaption>image-20220725154920071</figcaption></figure>
<p>此时，如果再来一个客户端Client3来尝试获取锁，它会在locks下再创建一个临时节点lock3</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220725154933777.png" alt="image-20220725154933777" tabindex="0" loading="lazy"><figcaption>image-20220725154933777</figcaption></figure>
<p>同样的，client3一样也会查找locks下面的所有临时顺序子节点，判断自己的节点lock3是不是最小的，发现自己不是最小的，就获取锁失败。它也是不会甘心的，它会向在它前面的节点lock2注册Watcher事件，以监听lock2节点是否存在。</p>
<figure><img src="@source/Java/eightpart/personal_images/image-20220725154939637.png" alt="image-20220725154939637" tabindex="0" loading="lazy"><figcaption>image-20220725154939637</figcaption></figure>
<h3 id="集群脑裂导致数据丢失怎么办" tabindex="-1"><a class="header-anchor" href="#集群脑裂导致数据丢失怎么办" aria-hidden="true">#</a> 集群脑裂导致数据丢失怎么办？</h3>
<blockquote>
<p>什么是脑裂？</p>
</blockquote>
