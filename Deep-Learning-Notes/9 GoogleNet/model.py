...
class Inception(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.cat(outputs, 1)  # 1：合并的维度
...
class InceptionAux(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        # 全连接
        self.fc1 = nn.Linear(2048, 1024)  # 上面的卷积层 128 * 4 * 4 = 2048
    ...
    def forward(self, x):
        ...
        x = self.conv(x)  # 128的卷积核
        # N x 128 x 4 x 4
        x = torch.flatten(x, 1)  # 通过维度1展平
        x = F.dropout(x, 0.5, training=self.training)  # 原文是实验70%的失活神经元，这里采样50%
        # N x 2048
